defmodule Dirup.Storage.AshObanIntegrationTest do
  use Dirup.DataCase
  use Oban.Testing, repo: Dirup.Repo

  alias Dirup.Storage
  alias Dirup.Storage.{StorageResource, ActorPersister}

  describe "AshOban integration" do
    setup do
      # Create a test user for actor persistence
      {:ok, user} =
        Dirup.Accounts.create_user(%{
    ...
    test "actor persister handles unknown actor lookup gracefully" do
      ...
      invalid_data = %{"type" => "Dirup.Accounts.User", "id" => "non-existent-id"}

      {:error, reason} = ActorPersister.lookup(invalid_data)
      assert reason =~ "User not found"
    end

    test "can schedule background storage processing job", %{
      user: user,
    ...
    test "can get job status for storage resource", %{
      user: user,
      storage_resource: storage_resource
    } do
    ...
    test "can cancel pending jobs for storage resource", %{
      user: user,
      storage_resource: storage_resource
    } do
    ...
    test "background job actions have proper authorization policies", %{
      user: user,
      storage_resource: storage_resource
    } do
      ...
      changeset =
        Ash.Changeset.for_update(storage_resource, :process_storage_async, %{
    ...
    test "background jobs preserve actor context", %{
      user: user,
      storage_resource: storage_resource
    } do
      # Schedule a job
