defmodule Kyozo.Storage.ObanIntegrationTest do
  ...
  describe "AshOban trigger integration" do
    test "schedule_storage_processing marks resource for processing" do
      ...
      {:ok, storage_resource} = StorageResource
    ...
    test "schedule_version_creation marks resource for version creation" do
      ...
      {:ok, updated_resource} = Storage.schedule_version_creation(
        storage_resource, 
        "new content",
        version_name: "v1.0",
    ...
    test "get_operation_stats returns correct statistics" do
      ...
      {:ok, _unprocessed} = StorageResource
      |> Ash.Changeset.for_create(:create, %{
      ...
      {:ok, _cleanup_scheduled} = StorageResource
      |> Ash.Changeset.for_create(:create, %{
    ...
    test "force_processing executes immediately" do
      ...
      |> Ash.Changeset.for_create(:create, %{
        ...
        file_size: 100,
        checksum: "abc123",
        storage_backend: :ram,  # Use RAM backend for testing
        storage_metadata: %{"processed" => false}
      })
      |> Ash.create()

      # Force processing (this should execute the action immediately)
      result = Storage.force_processing(
        storage_resource,
  ...
  describe "AshOban triggers" do
    ...
    test "triggers are properly configured" do
      ...
      |> Ash.Changeset.for_create(:create, %{
        ...
        checksum: "abc123",
        storage_backend: :disk,
        storage_metadata: %{}  # No processed flag means it needs processing
      })
      |> Ash.create()
      ...
      {:ok, cleanup_resource} = StorageResource
      |> Ash.Changeset.for_create(:create, %{
        file_name: "cleanup.txt",
        mime_type: "text/plain",
        file_size: 100,
      ...
      {:ok, version_resource} = StorageResource
      |> Ash.Changeset.for_create(:create, %{
