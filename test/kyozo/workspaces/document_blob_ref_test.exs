defmodule Dirup.Workspaces.DocumentBlobRefTest do
  ...
  describe "document blob reference creation" do
    ...
    test "prevents duplicate references for same document and ref_type", %{
      document: document,
      blob: blob
    } do
    ...
    test "allows multiple ref_types for same document", %{document: document, blob: blob} do
      {:ok, blob2} =
        Workspaces.create_blob(content: "Attachment content", content_type: "text/plain")
  ...
  describe "content retrieval and management" do
    ...
    test "updates document content", %{document: document} do
      ...
      assert {:ok, _ref} =
               DocumentBlobRef.update_document_content(
                 document.id,
                 new_content,
                 "text/markdown"
               )
  ...
  describe "content action helpers" do
    ...
    test "get_content action retrieves document content", %{document: document} do
      ...
      assert {:ok, retrieved_content} =
               Workspaces.get_document_blob_content(
                 document.id,
                 "content"
               )
    ...
    test "update_content action updates document content", %{document: document} do
      ...
      # Update using action
      assert {:ok, _updated_ref} =
               Workspaces.update_document_blob_content(
                 document.id,
                 updated_content,
                 "text/plain",
                 "content"
    ...
    test "handles content_type changes in updates", %{document: document} do
      ...
      {:ok, ref} =
        Workspaces.by_document_and_type(document.id, "content") |> elem(1) |> List.first()
  ...
  describe "helper function integration" do
    ...
    test "multiple reference types coexist" do
      ...
      assert {:ok, attachment_data} ==
               Workspaces.get_document_blob_content(
                 document.id,
                 "attachment"
               )

      assert {:ok, preview_html} ==
               Workspaces.get_document_blob_content(
                 document.id,
                 "preview"
               )
  ...
  describe "authorization and policies" do
    test "user can only access refs for documents they can access" do
      ...
      assert {:error, %Ash.Error.Forbidden{}} =
               Workspaces.get_document_blob_ref(ref2.id, actor: user1)
      ...
      assert {:error, %Ash.Error.Forbidden{}} =
               Workspaces.get_document_blob_ref(ref1.id, actor: user2)
    end
  end
end
