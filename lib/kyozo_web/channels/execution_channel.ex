defmodule KyozoWeb.ExecutionChannel do
  @moduledoc """
  ...
  alias Kyozo.{Accounts, Workspaces, Containers}
  alias KyozoWeb.Presence
  ...
  defp setup_subscriptions(socket) do
    case socket.assigns.resource_type do
      :notebook ->
        ...
        Phoenix.PubSub.subscribe(Kyozo.PubSub, "notebook_execution:#{socket.assigns.resource_id}")

      :container ->
        # Subscribe to container events
        Phoenix.PubSub.subscribe(Kyozo.PubSub, "container_events:#{socket.assigns.resource_id}")

      :file ->
        # Subscribe to file processing events
        Phoenix.PubSub.subscribe(Kyozo.PubSub, "file_processing:#{socket.assigns.resource_id}")

      :workspace ->
        # Subscribe to workspace-wide events
        Phoenix.PubSub.subscribe(Kyozo.PubSub, "workspace_events:#{socket.assigns.resource_id}")
    end
  end
  ...
  defp subscribe_to_logs(execution_id, socket) do
    ...
    Phoenix.PubSub.subscribe(Kyozo.PubSub, "execution_logs:#{execution_id}")
    :ok
  end
