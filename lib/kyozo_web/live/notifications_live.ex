defmodule KyozoWeb.NotificationsLive do
  use KyozoWeb, :live_view

  on_mount {KyozoWeb.LiveUserAuth, :current_user}

  def mount(_params, _session, socket) do
    notifications = Kyozo.Accounts.notifications_for_user!(actor: socket.assigns.current_user)

    if connected?(socket) do
      "notifications:#{socket.assigns.current_user.id}"
      |> KyozoWeb.Endpoint.subscribe()
    end
  ...
  def handle_event("dismiss-notification", %{"id" => id}, socket) do
    ...
    Kyozo.Accounts.dismiss_notification(
      notification,
