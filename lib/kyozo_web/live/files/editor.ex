defmodule KyozoWeb.Live.Workspaces.Files.Editor do
  @moduledoc """
    ...
    case load_document_with_content(document_id, user) do
      {:ok, document, content} ->
        ...
        if connected?(socket) do
          Phoenix.PubSub.subscribe(Kyozo.PubSub, "document:#{document_id}")
          Phoenix.PubSub.subscribe(Kyozo.PubSub, "document:#{document_id}:collaboration")
        end
    ...
    case save_document_content(socket, content) do
      {:ok, updated_document} ->
        socket =
          ...
          |> assign(:content, content)
          |> assign(:document_data, prepare_document_data(updated_document, content, socket.assigns.current_user))
  ...
  defp save_document_content(socket, content) do
    ...
    case DocumentBlobRef.update_document_content(
      ...
      {:ok, _blob_ref} ->
        ...
        case Workspaces.update_document(document, %{
