defmodule KyozoWeb.Live.Workspace.Dashboard do
  @moduledoc """
  ...
  alias Kyozo.Workspaces.{Workspace, File, Notebook}
  ...
  def handle_event("sort_files", %{"by" => sort_by}, socket) do
    current_order = if socket.assigns.sort_by == sort_by do
  ...
  defp load_workspace_data(workspace_id, user) do
    try do
      case Kyozo.Workspaces.get_workspace(workspace_id, actor: user, load: [:team, :created_by]) do
        {:ok, workspace} ->
          files = Kyozo.Workspaces.list_files(workspace_id: workspace_id, actor: user) || []
          notebooks = Kyozo.Workspaces.list_notebooks(workspace_id: workspace_id, actor: user) || []
  ...
  defp subscribe_to_workspace_events(workspace_id) do
    try do
      Phoenix.PubSub.subscribe(Kyozo.PubSub, "workspace:#{workspace_id}")
      Phoenix.PubSub.subscribe(Kyozo.PubSub, "files:#{workspace_id}")
      Phoenix.PubSub.subscribe(Kyozo.PubSub, "notebooks:#{workspace_id}")
    rescue
      _ -> nil
    end
  end
  ...
  defp build_breadcrumbs(path, workspace_name) do
    ...
    {_, breadcrumbs} = Enum.reduce(parts, {"/", base}, fn part, {current_path, acc} ->
  ...
  defp load_file_content(file) do
    try do
      case file do
        %File{} ->
          case Kyozo.Workspaces.get_file_content(file.id) do
            {:ok, content} -> content
            {:error, _} -> ""
          end
        %Notebook{} ->
          case Kyozo.Workspaces.get_notebook(file.id, load: [:content]) do
  ...
  defp create_file(workspace, file_params, user) do
    case file_params["file_type"] do
      ...
      "notebook" ->
        Kyozo.Workspaces.create_from_document(
          Map.merge(file_params, %{
            "workspace_id" => workspace.id,
  ...
  defp update_file(file, file_params, user) do
    try do
      case file do
        ...
        %Notebook{} ->
          Kyozo.Workspaces.update_content(
            file.id,
            Map.merge(file_params, %{
              "title" => file_params["name"],
  ...
  defp delete_file(file, user) do
    try do
      case file do
        %File{} ->
          Kyozo.Workspaces.delete_file(file.id, actor: user)
        
        %Notebook{} ->
          Kyozo.Workspaces.destroy_notebook(file.id, actor: user)
