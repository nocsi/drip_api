defmodule KyozoWeb.Live.Workspace.Index do
  @moduledoc """
  ...
  def mount(_, _, socket) do
    ...
    user_with_teams = if user && (!user.teams || match?(%Ash.NotLoaded{}, user.teams)) do
      try do
        Kyozo.Accounts.User
        |> Ash.get!(user.id, load: [:teams], actor: user, domain: Kyozo.Accounts)
      rescue
        _ -> user
      end
    else
    ...
    teams = if user_with_teams && user_with_teams.teams do
    ...
    workspaces = if teams != [] && length(teams) > 0 do
      team = List.first(teams)
      try do
        # Try to get workspaces using the team as tenant
        case Kyozo.Workspaces.list_active_workspaces(actor: user_with_teams, tenant: team) do
          {:ok, ws} -> ws
          _ -> []
        end
      rescue
    ...
    if connected?(socket) && socket.assigns.current_team do
      try do
        Phoenix.PubSub.subscribe(Kyozo.PubSub, "workspace:#{socket.assigns.current_team.id}")
      rescue
        _ -> nil
      end
    end
  ...
  defp apply_action(socket, action, %{"id" => id})
    ...
    try do
      case Kyozo.Workspaces.get_workspace(id, actor: socket.assigns.current_user) do
        {:ok, workspace} ->
          assign(socket, workspace: workspace)
        {:error, _} ->
          socket
  ...
  def handle_event("delete_workspace", %{"id" => id}, socket) do
    try do
      case Kyozo.Workspaces.delete_workspace(id, actor: socket.assigns.current_user) do
        {:ok, workspace} ->
          socket =
  ...
  def handle_event("archive_workspace", %{"id" => id}, socket) do
    try do
      case Kyozo.Workspaces.archive_workspace(id, actor: socket.assigns.current_user) do
        {:ok, workspace} ->
          socket =
            stream_delete(socket, :workspaces, workspace)
  ...
  def handle_event("duplicate_workspace", %{"id" => id}, socket) do
    try do
      case Kyozo.Workspaces.get_workspace(id, actor: socket.assigns.current_user) do
        {:ok, workspace} ->
          case Kyozo.Workspaces.duplicate_workspace(
            workspace,
            new_name: nil,
  ...
  @impl true
  def handle_info({KyozoWeb.Live.Workspace.Form, {:saved, workspace}}, socket) do
