defmodule Mix.Tasks.GenerateFromOpenapi do
  ...
  def run([openapi_path]) do
    ...
    Mix.shell().info(
      "Generating domain #{domain_name} with #{map_size(filtered_schemas)} resources..."
    )
    ...
    Mix.shell().info(
      "  1. Review generated resources in lib/#{Macro.underscore(app)}/#{Macro.underscore(domain_name)}/"
    )
  ...
  def run(_) do
    ...
    Mix.shell().info(
      "Example: mix generate_from_openapi priv/openapi/kyozo/workspace/v1/workspace.openapi.yaml"
    )
  end
  ...
  defp extract_domain_info(openapi_spec, app_module) do
    ...
    domain_parts =
      title
      |> String.split(".")
      # Take first two parts: "kyozo.workspace"
      |> Enum.take(2)
      |> Enum.map(&Macro.camelize/1)

    domain_name =
      case domain_parts do
        [_app, domain] -> domain
        [domain] -> domain
        [] -> "Api"
  ...
  defp should_include_schema?({schema_name, _schema_struct}) do
    ...
    not (String.starts_with?(name_str, "connect") or
           ...
           String.ends_with?(name_str, "Entry") or
  ...
  defp should_generate_resource?(schema_struct) do
    case schema_struct do
      ...
      %{"type" => type, "const" => _} when type in ["string", "number", "integer", "boolean"] ->
        false

      # Skip primitives without properties
      %{"type" => type} when type in ["string", "number", "integer", "boolean"] ->
        Map.has_key?(schema_struct, "properties") and
          map_size(Map.get(schema_struct, "properties", %{})) > 0
    ...
    if resource_already_exists?(resource_module) do
      ...
      try do
        ...
        case result do
          ...
          # ash.gen.resource doesn't always return :ok
          _ -> {:ok, resource_module}
        end
      rescue
        error ->
  ...
  defp resolve_schema(%{"$ref" => ref_path}, openapi_spec) do
    # Remove leading "#"
    ref_parts = String.split(ref_path, "/") |> Enum.drop(1)
  ...
  defp map_openapi_type_to_ash(%{"type" => "string", "format" => "date-time"}),
  ...
  # Fallback
  defp map_openapi_type_to_ash(_), do: "string"
  ...
  defp infer_actions_from_paths(paths, clean_name, full_resource_name) do
    ...
    rest_patterns = [
      # Plural
      "/#{Macro.underscore(clean_name)}s",
      # Singular
      "/#{Macro.underscore(clean_name)}",
      # Lowercase plural
      "/#{String.downcase(clean_name)}s",
      # Lowercase singular
    ...
    # Find matching paths
    matching_paths =
      Enum.filter(paths, fn {path, _ops} ->
    ...
    actions =
      if Enum.any?(all_ops, &(&1 in ["patch", "put"])), do: ["update" | actions], else: actions
