defmodule Mix.Tasks.Dirup.Bootstrap do
  ...
  @shortdoc "Bootstrap Kyozo with all required data"
  ...
  defp ensure_admin_user do
    ...
    email = System.get_env("ADMIN_EMAIL", "admin@kyozo.store")
  ...
  defp verify_setup do
    ...
    {:ok, result} =
      Ecto.Adapters.SQL.query(Dirup.Repo, """
        ...
        FROM users
        WHERE email LIKE '%admin%' OR email LIKE '%kyozo%'
        ORDER BY email
        LIMIT 5;
      """)
    ...
    Logger.info("=" |> String.duplicate(50))
    Logger.info("\n🎉 Setup complete!")
    Logger.info("   Admin Email: admin@kyozo.store")
