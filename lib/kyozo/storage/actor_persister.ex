defmodule Kyozo.Storage.ActorPersister do
  @moduledoc """
  ...
  def store(%{__struct__: user_module, id: id} = _actor)
      when user_module in [Kyozo.Accounts.User, Kyozo.Accounts.ApiKey] do
    %{
  ...
  def lookup(%{"type" => user_type, "id" => id})
      when user_type in ["Kyozo.Accounts.User"] do
    case Kyozo.Accounts.get_user_by_id(id) do
      {:ok, user} -> {:ok, user}
  ...
  def lookup(%{"type" => "Kyozo.Accounts.ApiKey", "id" => id}) do
    case Kyozo.Accounts.get_api_key_by_id(id) do
