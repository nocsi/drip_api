defmodule Kyozo.Storage.GitNif do
  @moduledoc """
  ...
  alias Kyozo.Storage.Git.{Repository, Utils}
  require Logger
  ...
  def read_file_content(repo_path, file_path) when is_binary(repo_path) and is_binary(file_path) do
  ...
  def write_and_commit_file(repo_path, file_path, content, opts \\ []) do
    ...
    with {:ok, repo} <- ensure_repository_opened(repo_path),
         ...
         {:ok, commit_sha} <- create_commit(repo, tree_sha, commit_message, author_name, author_email, parents),
  ...
  defp create_tree_without_file(repo, file_path) do
    case get_current_tree_entries(repo) do
      {:ok, current_entries} ->
        filtered_entries = Enum.reject(current_entries, fn {path, _sha, _mode} ->
          path == file_path
        end)
  ...
  defp get_current_tree_entries(repo) do
    case Repository.get_head(repo) do
      ...
      {:error, :head_not_found} -> {:error, :empty_repository}
      {:error, :ref_not_found} -> {:error, :empty_repository}
      error -> error
    end
  end
  ...
  defp flatten_tree_entries(repo, tree_sha, prefix) do
    case Repository.read_tree(repo, tree_sha) do
      {:ok, entries} ->
        flat_entries = Enum.flat_map(entries, fn entry ->
  ...
  defp create_tree_recursive(repo, tree_structure) do
    entries = Enum.map(tree_structure, fn
      {name, {sha, mode}} ->
        # File entry
  ...
  defp get_object_statistics(repo) do
    ...
    case Kyozo.Storage.Git.Object.get_stats(git_dir) do
      {:ok, stats} -> {:ok, stats}
      error -> error
    end
  end
  ...
  defp get_ref_statistics(repo) do
    ...
    with {:ok, branches} <- Kyozo.Storage.Git.Ref.list_branches(git_dir),
         {:ok, tags} <- Kyozo.Storage.Git.Ref.list_tags(git_dir) do
