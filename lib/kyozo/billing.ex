defmodule Kyozo.Billing do
  use Ash.Domain, otp_app: :kyozo
  require Logger

  resources do
    resource Kyozo.Billing.Customer do
    ...
    resource Kyozo.Billing.Subscription do
    ...
    resource Kyozo.Billing.Usage do
      define :record_usage
    ...
    resource Kyozo.Billing.Invoice do
    ...
    resource Kyozo.Billing.Plan do
  ...
  def subscribe(topic) when is_binary(topic) do
    Phoenix.PubSub.subscribe(Kyozo.PubSub, "billing:#{topic}")
  end
  ...
  def broadcast(topic, event, payload) do
    Phoenix.PubSub.broadcast(Kyozo.PubSub, "billing:#{topic}", {event, payload})
  end
    ...
    unit_price =
      case usage_data.service do
        ...
        "polyglot_translation" ->
          # $0.02 per KB
          kb_size = max(1, div(usage_data.content_size_bytes, 1024))
  ...
  def create_subscription_from_apple_receipt(customer, receipt_info) do
    ...
    subscription_attrs = %{
      ...
      plan_id: plan.id,
      status:
        if(Kyozo.Billing.AppleReceiptValidator.subscription_active?(receipt_info),
          do: :active,
          else: :expired
        ),
      provider: :apple,
  ...
  def handle_apple_notification(notification_payload) do
    case Kyozo.Billing.AppleReceiptValidator.validate_notification(notification_payload) do
  ...
  def sync_subscription_with_apple(subscription, receipt_data) do
    case Kyozo.Billing.AppleReceiptValidator.validate_and_parse(receipt_data) do
      {:ok, receipt_info} ->
        update_attrs = %{
          status:
            if(Kyozo.Billing.AppleReceiptValidator.subscription_active?(receipt_info),
