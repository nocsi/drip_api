defmodule Kyozo.Projects.LoadEvent do
  use Ash.Resource,
    otp_app: :kyozo,
  ...
  attributes do
    ...
    attribute :task_runme_id, :string do
      public? true
      description "Task runme ID if event_type is :found_task"
    end
    ...
    attribute :sequence_number, :integer do
      ...
      public? true
      default 0
      description "Order of this event within the project loading sequence"
    end
  ...
  relationships do
    ...
    belongs_to :document, Kyozo.Projects.Document do
      public? true
      description "Document related to this event (for parsing events)"
    end

    belongs_to :task, Kyozo.Projects.Task do
      public? true
  ...
  actions do
    ...
    create :log_found_task do
      accept [
        :path, :task_name, :task_runme_id, :is_task_name_generated,
        :sequence_number, :processing_time_ms, :event_data
      ]
    ...
    create :create_generic do
      accept [
        :event_type, :event_data, :path, :error_message, :task_name,
        :task_runme_id, :is_task_name_generated, :sequence_number,
        :processing_time_ms, :occurred_at
      ]
      ...
      calculation fn records, _opts ->
        Enum.map(records, fn record ->
          case record.event_type do
            ...
            :started_walk -> 
              "Started walking project"
            :finished_walk -> 
              "Finished walking project"
            _ -> 
              "#{record.event_type}"
          end
        end)
  ...
  aggregates do
    count :events_count, :project
  end

  identities do
    identity :unique_project_sequence, [:project_id, :sequence_number] do
      message "An event with this sequence number already exists for this project"
    end
  end
  ...
  validations do
    ...
    validate present(:path) do
      where attribute_in(:event_type, [:found_dir, :found_file, :started_parsing_doc, :finished_parsing_doc])
