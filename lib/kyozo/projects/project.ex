defmodule Kyozo.Projects.Project do
  use Ash.Resource,
    otp_app: :kyozo,
  ...
  relationships do
    ...
    has_many :documents, Kyozo.Projects.Document do
      description "Documents found in this project"
    end

    has_many :tasks, Kyozo.Projects.Task do
  ...
  actions do
    ...
    create :load_directory do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.change_attribute(:identity_mode, Ash.Changeset.get_argument(changeset, :identity))
        |> Ash.Changeset.change_attribute(:options, options)
      end

      change {Kyozo.Projects.Changes.LoadProject, type: :directory}
    end
    ...
    create :load_file do
      ...
      change {Kyozo.Projects.Changes.LoadProject, type: :file}
    end
    ...
    create :load_project do
      ...
      change {Kyozo.Projects.Changes.LoadProject, type: :auto}
    end
      ...
      calculation fn records, _opts ->
        Enum.map(records, fn record ->
          case record.status do
            :loaded -> 1.0
            :error -> 0.0
            :loading -> 0.5  # Could be more sophisticated based on events
          end
        end)
      end
    end
  ...
  validations do
    ...
    validate {Kyozo.Projects.Validations.ValidPath, []} do
      where changing(:path)
