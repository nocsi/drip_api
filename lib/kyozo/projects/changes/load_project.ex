defmodule Kyozo.Projects.Changes.LoadProject do
  use Ash.Resource.Change

  alias Kyozo.Projects.Services.ProjectLoader
  ...
  defp load_project(project, type, context) do
    ...
    case type do
      :directory -> load_directory_project(project, sequence_counter, context)
      :file -> load_file_project(project, sequence_counter, context)
      :auto -> 
  ...
  defp load_directory_project(project, sequence_counter, context) do
    ...
    try do
      ...
      files = ProjectLoader.walk_directory(
        project.path,
      ...
      {documents, tasks} = process_markdown_files(project, markdown_files, sequence_counter, context)
      
      # Log completion
      log_event(project, :finished_walk, %{
        files_found: length(files),
        markdown_files: length(markdown_files),
        documents_created: length(documents),
        tasks_found: length(tasks)
      }, sequence_counter, context)
  ...
  defp load_file_project(project, sequence_counter, context) do
    try do
      ...
      {documents, tasks} = process_markdown_files(project, [project.path], sequence_counter, context)
  ...
  defp process_single_file(project, file_path, sequence_counter, context) do
    ...
    try do
      ...
      {:ok, document} = 
        Kyozo.Projects.Document
        |> Ash.Changeset.for_create(:create, document_attrs, actor: project)
        |> Kyozo.Projects.create()
      
      # Parse markdown to find tasks
      tasks = ProjectLoader.extract_tasks_from_markdown(
        content, 
      ...
      created_tasks = 
        ...
        |> Enum.map(fn {task_data, index} ->
          task_attrs = Map.merge(task_data, %{
            order_index: index,
      ...
      log_event(project, :finished_parsing_doc, %{
        path: relative_path,
        tasks_found: length(created_tasks)
      }, sequence_counter, context, %{document_id: document.id})
