defmodule Kyozo.Workspaces.Workspace.Changes.DuplicateWorkspace do
  @moduledoc """
      ...
      {:ok, duplicate_workspace} ->
        ...
        case execute_copy_tasks(copy_tasks, context) do
          ...
          {:error, error} ->
            ...
            Ash.Changeset.add_error(changeset, "Failed to copy workspace content: #{inspect(error)}")
        end

      {:error, error} ->
        Ash.Changeset.add_error(changeset, "Failed to create duplicate workspace: #{inspect(error)}")
    end
  end
  ...
  defp create_duplicate_workspace(attrs, context) do
    ...
    case Kyozo.Workspaces.create_workspace(
      attrs,
      actor: context.actor,
  ...
  defp copy_workspace_content(:documents, source_workspace_id, target_workspace_id, context) do
    ...
    case Kyozo.Workspaces.list_documents(
  ...
  defp copy_workspace_content(:notebooks, source_workspace_id, target_workspace_id, context) do
    ...
    case Kyozo.Workspaces.list_notebooks(
      query: [filter: [workspace_id: source_workspace_id]],
      actor: context.actor,
      tenant: context.tenant
    ) do
  ...
  defp copy_documents([document | rest], target_workspace_id, context) do
    case Kyozo.Workspaces.duplicate_document(
      document,
      new_title: nil,
  ...
  defp copy_notebooks([notebook | rest], target_workspace_id, context) do
    case Kyozo.Workspaces.duplicate_notebook(
      notebook,
      new_title: nil,
      copy_to_workspace_id: target_workspace_id,
      actor: context.actor,
  ...
  defp clean_up_workspace(workspace, context) do
    ...
    case Kyozo.Workspaces.delete_workspace(
      workspace.id,
      actor: context.actor,
      tenant: context.tenant
    ) do
      {:ok, _} -> :ok
      {:error, _} -> :error # We tried our best to clean up
    end
  end
end