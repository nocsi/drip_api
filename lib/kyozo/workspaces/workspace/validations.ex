defmodule Kyozo.Workspaces.Workspace.Validations do
  @moduledoc """
  ...
  defmodule ValidateSettings do
    ...
    defp validate_required_settings(settings) do
      ...
      if Enum.empty?(missing_keys) do
        ...
        {:error, field: :settings, message: "Missing required settings: #{Enum.join(missing_keys, ", ")}"}
      end
    end
    ...
    defp validate_max_file_size(settings) do
      case Map.get(settings, "max_file_size") do
        nil -> :ok
        size when is_integer(size) and size > 0 and size <= 1_073_741_824 -> :ok  # Max 1GB
    ...
    defp validate_backup_frequency(settings) do
      case Map.get(settings, "backup_frequency") do
        nil -> :ok
        freq when freq in ["hourly", "daily", "weekly", "monthly"] -> :ok
        freq when is_binary(freq) ->
          {:error, field: :settings, message: "backup_frequency must be one of: hourly, daily, weekly, monthly"}
        _ ->
  ...
  defmodule UniqueNamePerTeam do
    ...
    def validate(changeset, _opts, _context) do
      ...
      if name && team_id do
        case check_name_uniqueness(name, team_id, workspace_id) do
          true -> :ok
          false -> {:error, field: :name, message: "A workspace with this name already exists in your team"}
        end
      else
        :ok
      end
    ...
    defp check_name_uniqueness(name, team_id, workspace_id) do
      query = Kyozo.Workspaces.Workspace
      ...
      case Kyozo.Workspaces.read_one(query) do
        {:ok, nil} -> true
        {:ok, _workspace} -> false
        {:error, _} -> true  # Assume unique if we can't check
      end
    end
  end
  ...
  defmodule ValidateWorkspaceAccess do
    ...
    def validate(changeset, _opts, context) do
      ...
      if team_id && actor do
        case check_team_membership(actor, team_id) do
          true -> :ok
          false -> {:error, field: :team_id, message: "You don't have permission to create workspaces in this team"}
        end
      else
    ...
    defp check_team_membership(actor, team_id) do
      case Kyozo.Workspaces.is_member?(actor.id, team_id) do
        {:ok, true} -> true
        _ -> false
      end
    rescue
  ...
  defmodule ValidateStorageQuota do
    ...
    def validate(changeset, _opts, _context) do
      ...
      if team_id do
        max_workspace_size = Map.get(settings, "max_workspace_size", 1_073_741_824)  # 1GB default
  ...
  defmodule ValidateWorkspaceName do
    ...
    defp validate_characters(name) do
      if Regex.match?(~r/^[a-zA-Z0-9\-_\s]+$/, name) do
        ...
        {:error, field: :name, message: "Name can only contain letters, numbers, spaces, hyphens, and underscores"}
      end
    end
    ...
    defp validate_reserved_names(name) do
      reserved_names = [
        ...
        "system", "null", "undefined", "default", "config", ".git",
        "node_modules", "tmp", "temp", "cache", "log", "logs"
      ]
