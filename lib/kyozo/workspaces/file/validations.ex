defmodule Kyozo.Workspaces.File.Validations do
  @moduledoc """
    ...
    defp validate_path(file_path) do
      cond do
        String.contains?(file_path, "..") ->
          {:error, field: :file_path, message: "File path cannot contain '..' (directory traversal)"}
  ...
  defmodule ValidateContentType do
    ...
    @supported_types [
      ...
      "application/x-jupyter-notebook", "application/javascript",
      "text/x-python", "text/x-r", "text/x-julia", "text/x-sql",
    ...
    defp validate_content_type(content_type) do
      cond do
        ...
        true ->
          {:error,
           field: :content_type,
           message: "Unsupported or invalid content type: #{content_type}"}
      end
    end
  ...
  defmodule ValidateStorageBackend do
    ...
    defp is_large_file?(file_path) do
      ...
      large_extensions = [".zip", ".tar", ".gz", ".pdf", ".mp4", ".avi",
                         ".jpg", ".jpeg", ".png", ".gif", ".dmg", ".iso"]
    ...
    defp is_version_controlled_file?(content_type, file_path) do
      ...
      vc_content_types = ["text/markdown", "application/x-jupyter-notebook",
                         "text/x-python", "text/x-r", "text/x-julia",
                         "application/json", "text/yaml"]

      vc_extensions = [".md", ".ipynb", ".py", ".js", ".ts", ".r", ".jl",
  ...
  defmodule ValidateFileSize do
    ...
    defp validate_size_for_backend(size, backend) do
      ...
      if size > max_size do
        {:error,
         field: :file_size,
         message: "File size (#{format_bytes(size)}) exceeds maximum for #{backend} backend (#{format_bytes(max_size)})"}
      else
        :ok
      end
    end
    ...
    defp validate_title(title) do
      cond do
        ...
        String.match?(title, ~r/[\/\\:*?"<>|]/) ->
          {:error, field: :title, message: "Title contains invalid characters: / \\ : * ? \" < > |"}
        ...
        title in ["CON", "PRN", "AUX", "NUL", "COM1", "COM2", "COM3", "COM4",
  ...
  defmodule ValidateUniqueFilePath do
    ...
    def validate(changeset, _opts, _context) do
      team_id = Ash.Changeset.get_attribute(changeset, :team_id)
      file_path = Ash.Changeset.get_attribute(changeset, :file_path)
      document_id = case Ash.Changeset.get_data(changeset) do
        %{id: id} -> id
        _ -> nil
      end
    ...
    def validate(changeset, _opts, _context) do
      ...
      cond do
        is_directory && content_type != "application/x-directory" ->
          {:error, field: :content_type, message: "Directories must have content_type 'application/x-directory'"}

        not is_directory && content_type == "application/x-directory" ->
          {:error, field: :is_directory, message: "Only directories can have content_type 'application/x-directory'"}
  ...
  defmodule ValidateParentDirectory do
    ...
    defp validate_parent_exists_and_is_directory(parent_id, context) do
      ...
      case Kyozo.Workspaces.get_file(parent_id, actor: actor) do
        {:ok, parent_file} ->
