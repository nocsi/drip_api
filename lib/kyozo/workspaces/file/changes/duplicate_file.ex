defmodule Kyozo.Workspaces.File.Changes.DuplicateFile do
  @moduledoc """
  ...
  def change(changeset, _opts, context) do
    ...
    case get_file_content(source_file) do
      ...
      {:error, error} ->
        Ash.Changeset.add_error(changeset, "Failed to read source file content: #{inspect(error)}")
    end
  end
  ...
  defp generate_duplicate_name(original_name) do
    ...
    {base_name, ext} = case Path.extname(original_name) do
      "" -> {original_name, ""}
  ...
  defp create_duplicate_file(attrs, content, commit_message, context) do
    ...
    case Kyozo.Workspaces.create_file(
