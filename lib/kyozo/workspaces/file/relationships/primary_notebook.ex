defmodule Kyozo.Workspaces.File.Relationships.PrimaryNotebook do
  @moduledoc """
  ...
  alias Kyozo.Workspaces.{FileNotebook, Notebook}
  ...
  def load(files, _opts, _context) do
    ...
    all_file_notebooks = FileNotebook
    ...
    notebook_ids = all_file_notebooks
    |> Enum.map(& &1.notebook_id)
    |> Enum.reject(&is_nil/1)

    # Load the Notebook resources
    notebook_resources = if length(notebook_ids) > 0 do
      Notebook
    ...
    notebooks_by_file = all_file_notebooks
    ...
    |> Enum.map(fn {file_id, file_notebook_list} ->
      ...
      notebook = primary_file_notebook && primary_file_notebook.notebook_id && Map.get(notebook_lookup, primary_file_notebook.notebook_id)
      {file_id, notebook}
    end)
    |> Enum.into(%{})
