defmodule Kyozo.Workspaces.File.Changes do
  @moduledoc """
    ...
    def change(changeset, _opts, _context) do
      case Ash.Changeset.get_attribute(changeset, :file_path) do
        nil ->
          ...
          if name do
            file_path = Kyozo.Workspaces.File.build_file_path(name, content_type)
  ...
  defmodule DetectBinaryContent do
    ...
    def change(changeset, _opts, context) do
      case get_content_from_args(changeset, context) do
        nil -> changeset
        content ->
          is_binary = Kyozo.Workspaces.File.binary_content?(content)
        ...
        content ->
          ...
          if size > @max_file_size do
            Ash.Changeset.add_error(changeset,
              field: :content,
              message: "File size (#{format_bytes(size)}) exceeds maximum allowed size (#{format_bytes(@max_file_size)})"
            )
          else
  ...
  defmodule CreateFileStorage do
    ...
    def change(changeset, _opts, context) do
      ...
      |> Ash.Changeset.before_transaction(fn changeset, _context ->
        with {:ok, content} <- get_content(changeset),
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset, message: "Failed to store file: #{inspect(reason)}")
        end
      end)
    end
    ...
    defp build_storage_options(changeset, context) do
      ...
      options = [
        workspace_id: team_id,  # Using team_id as workspace_id for now
        team_id: team_id,
        author: get_actor_name(context),
        commit_message: Ash.Changeset.get_argument(changeset, :initial_commit_message) || "Create document"
      ]
  ...
  defmodule UpdateFileStorage do
    ...
    def change(changeset, _opts, context) do
      ...
      |> Ash.Changeset.before_transaction(fn changeset, _context ->
        with {:ok, content} <- get_update_content(changeset),
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset, message: "Failed to update file: #{inspect(reason)}")
        end
      end)
    end
    ...
    defp build_storage_options(changeset, context) do
      ...
      options = [
        ...
        commit_message: Ash.Changeset.get_argument(changeset, :commit_message) || "Update document"
      ]
  ...
  defmodule DeleteFileStorage do
    ...
    def change(changeset, opts, context) do
      ...
      if soft_delete do
        changeset  # For soft delete, we keep the file in storage
      else
        changeset
    ...
    defp delete_from_storage(record, context) do
      ...
      case provider.delete(record.file_path, options) do
        ...
        {:error, reason} ->
          ...
          :ok  # Don't fail the transaction for storage deletion errors
      end
    end
  ...
  defmodule RenameFile do
    ...
    defp build_new_file_path(changeset, new_title) do
      data = Ash.Changeset.get_data(changeset)
      new_path = Kyozo.Workspaces.File.build_file_path(new_title, data.content_type)
      {:ok, new_path}
    end
  ...
  defmodule RenderFile do
    ...
    def run(input, _opts, context) do
      ...
      with {:ok, content} <- get_file_content(input, context),
        ...
        cache_key = "#{target_format}_#{:crypto.hash(:md5, inspect(options)) |> Base.encode16(case: :lower)}"
        cache_entry = %{
          content: rendered,
    ...
    defp render_content(content, source_format, target_format, _options) do
      ...
      case {source_format, target_format} do
        {"text/markdown", :html} ->
          {:ok, "<html><body>#{content}</body></html>"}  # Mock HTML rendering

        {"application/x-jupyter-notebook", :html} ->
          {:ok, "<html><body>Notebook: #{content}</body></html>"}  # Mock notebook rendering

        {_, :html} ->
          {:ok, "<html><body><pre>#{content}</pre></body></html>"}  # Default HTML rendering
  ...
  defmodule CreatePrimaryStorage do
    ...
    def change(changeset, _opts, context) do
      ...
      |> Ash.Changeset.before_transaction(fn changeset, context ->
        with {:ok, content} <- get_content(changeset),
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset, message: "Failed to create primary storage: #{inspect(reason)}")
        end
      end)
    end
    ...
    defp sync_storage_metadata_to_file(changeset, storage_entry) do
      ...
      |> Ash.Changeset.change_attribute(:storage_metadata, storage_resource.storage_metadata || %{})
    end
    ...
    defp determine_storage_backend(%{"content" => content, "filename" => filename}) do
      cond do
        ...
        byte_size(content) > 10 * 1024 * 1024 -> :s3  # > 10MB
  ...
  defmodule UpdatePrimaryStorage do
    ...
    def change(changeset, _opts, context) do
      ...
      |> Ash.Changeset.before_transaction(fn changeset, context ->
        ...
        with {:ok, content} <- get_update_content(changeset),
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset, message: "Failed to update primary storage: #{inspect(reason)}")
        end
      end)
    end
    ...
    defp update_storage_content(storage, content, context) do
      ...
      case Kyozo.Storage.StorageResource.upload_new_version(storage.id, %{
    ...
    defmodule CreateSpecializedResource do
      ...
      def change(changeset, _opts, _context) do
        ...
        |> Ash.Changeset.after_action(fn changeset, file ->
          if Kyozo.Workspaces.File.should_resolve_to_specialized?(file) do
            case create_intermediary_resource(file, changeset) do
              ...
              {:error, reason} ->
                require Logger
                Logger.warning("Failed to create specialized resource for file #{file.id}: #{inspect(reason)}")
                {:ok, file}
            end
          else
            {:ok, file}
      ...
      defp create_intermediary_resource(file, changeset) do
        ...
        intermediary_module = Kyozo.Workspaces.File.get_intermediary_module(file)
    ...
    defmodule ResolveToMedia do
      ...
      def run(file, input, _context) do
        ...
        case Ash.load(file, :primary_file_media) do
          {:ok, %{primary_file_media: %{media: media}}} when not is_nil(media) and not force_create ->
            {:ok, media}
      ...
      defp create_or_find_media_resource(file) do
        storage_type = Kyozo.Workspaces.FileTypeMapper.determine_storage_type(file.name, file.content_type)
      ...
      defp get_file_content(file) do
        ...
        case Ash.load(file, :primary_storage) do
          {:ok, %{primary_storage: storage}} when not is_nil(storage) ->
            Kyozo.Storage.retrieve_content(storage)
    ...
    defmodule ResolveToNotebook do
      ...
      def run(file, input, _context) do
        ...
        case Ash.load(file, :primary_file_notebook) do
          {:ok, %{primary_file_notebook: %{notebook: notebook}}} when not is_nil(notebook) and not force_create ->
            {:ok, notebook}
  ...
  #     case Kyozo.Workspaces.FileStorage.create_file_storage(%{
  #       file_id: file.id,
  ...
  #     case Kyozo.Workspaces.DocumentStorage.by_document_and_storage(document_id, storage_id) do
  ...
  #     case Kyozo.Workspaces.DocumentStorage.set_as_primary(relationship.id) do
  ...
  #     case Kyozo.Storage.get_storage(relationship.storage_id) do
  #       {:ok, storage} ->
  #         document
  #         |> Ash.Changeset.for_update(:update)
  #         |> Ash.Changeset.change_attribute(:file_size, storage.file_size)
