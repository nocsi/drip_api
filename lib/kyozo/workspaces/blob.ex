defmodule Kyozo.Workspaces.Blob do
        ...
        if content do
          ...
          case __MODULE__ |> Ash.Query.filter(hash == ^hash) |> Ash.read_one() do
            {:ok, existing_blob} when not is_nil(existing_blob) ->
              ...
              Ash.Changeset.add_error(changeset, field: :hash, message: "exists", value: existing_blob)
  ...
  policies do
    policy action_type(:read) do
      authorize_if always()  # Blobs are content-addressable and can be read by anyone who knows the hash
    end
  ...
  attributes do
    ...
    attribute :hash, :string do
      allow_nil? false
      public? true
      constraints min_length: 64, max_length: 64  # SHA-256 hash
  ...
  relationships do
    ...
    # has_many :document_blob_refs, Kyozo.Workspaces.DocumentBlobRef do
    #   destination_attribute :blob_id
    # end

    # many_to_many :files, Kyozo.Workspaces.File do
    #   through Kyozo.Workspaces.DocumentBlobRef
  ...
  calculations do
    calculate :reference_count, :integer do
      calculation fn blobs, _context ->
        ...
        # counts = Kyozo.Workspaces.DocumentBlobRef
        ...
        Enum.map(blobs, fn _blob ->
          0  # Return 0 for now until blob system is updated
        end)
      end
    end
  ...
  def store_content(hash, content) do
    ...
    backend = Application.get_env(:kyozo, :blob_storage_backend, :disk)
  ...
  defp s3_config do
    Application.get_env(:kyozo, :s3_storage, [])
  end

  defp blob_storage_root do
    Application.get_env(:kyozo, :blob_storage_root, 
  ...
  def create_or_find(content, content_type \\ "application/octet-stream", encoding \\ "utf-8") do
    ...
    case __MODULE__ |> Ash.Query.filter(hash == ^hash) |> Ash.read_one() do
      ...
      {:ok, nil} ->
        case Ash.create(__MODULE__, %{
          content: content,
          content_type: content_type,
          encoding: encoding
        }, action: :create_blob) do
          {:ok, blob} -> {:ok, blob}
          {:error, reason} -> {:error, reason}
        end
