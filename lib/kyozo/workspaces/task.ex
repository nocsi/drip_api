defmodule Kyozo.Workspaces.Task do
  use Ash.Resource,
    otp_app: :kyozo,
  ...
  postgres do
    ...
    identity_wheres_to_sql [
      unique_notebook_workspace_id: "workspace_id IS NOT NULL"
    ]
  end
  ...
  attributes do
    ...
    attribute :workspace_id, :uuid do
      public? true
      description "Unique identifier from workspace parsing"
    end
    ...
    attribute :line_start, :integer do
      public? true
      description "Starting line number in the notebook"
    end

    attribute :line_end, :integer do
      public? true
      description "Ending line number in the notebook"
    end
    ...
    attribute :order_index, :integer do
      ...
      public? true
      default 0
      description "Order of this task within the notebook"
    end
  ...
  relationships do
    ...
    belongs_to :notebook, Kyozo.Workspaces.Notebook do
      allow_nil? false
      public? true
      description "Notebook containing this task"
    end
  ...
  actions do
    ...
    read :list_notebook_tasks do
      argument :notebook_id, :uuid do
        allow_nil? false
      end

      filter expr(notebook_id == ^arg(:notebook_id))
      prepare build(sort: [order_index: :asc])
    end

    read :list_workspace_tasks do
      argument :workspace_id, :uuid do
        allow_nil? false
      end

      filter expr(workspace_id == ^arg(:workspace_id))
      prepare build(sort: [order_index: :asc], load: [:notebook])
    end
    ...
    create :create do
      accept [
        :workspace_id, :name, :is_name_generated, :language, :code, :description,
  ...
  calculations do
    ...
    calculate :full_path, :string do
      description "Full path including notebook path and task name"
      load [:notebook]

      calculation fn records, _opts ->
        Enum.map(records, fn record ->
          if record.notebook do
            "#{record.notebook.path}##{record.name}"
          else
            record.name
          end
        end)
  ...
  aggregates do
    count :total_executions, :workspace
  end

  identities do
    identity :unique_notebook_workspace_id, [:notebook_id, :workspace_id] do
      where expr(not is_nil(workspace_id))
      message "A task with this workspace_id already exists in the notebook"
    end

    identity :unique_notebook_order, [:notebook_id, :order_index] do
      message "A task with this order already exists in the notebook"
    end
  end
