defmodule Kyozo.Workspaces.FileNotebook do
  @derive {Jason.Encoder, only: [:id, :file_id, :notebook_id, :storage_resource_id, :relationship_type, :is_primary, :metadata, :created_at, :updated_at]}
  ...
  def supported_mime_types do
    [
      ...
      "application/json", # fallback for .ipynb files
  ...
  def validate_content(content, metadata) do
    ...
    cond do
      ...
      String.starts_with?(mime_type, "application/x-ipynb") and not valid_jupyter_notebook?(content) ->
    ...
    case backend do
      ...
      :hybrid ->
        Map.merge(base_options, %{
          ...
          split_outputs: true, # Store outputs separately in S3
          ...
          case Ash.get(Kyozo.Workspaces.File, file_id) do
            {:ok, file} ->
              ...
              case Kyozo.Storage.store_content(content, file.name,
                     ...
                     storage_options: %{notebook_format: notebook_format}) do
                {:ok, storage_resource} ->
                  changeset
                ...
                {:error, reason} ->
                  Ash.Changeset.add_error(changeset, "Failed to store notebook content: #{inspect(reason)}")
              end
