defmodule Kyozo.Workspaces.Extensions.RenderMarkdown.PostProcessors.TaskExtractor do
  @moduledoc """
        ...
        case element do
          ...
          {"pre", attrs, [{"code", code_attrs, code_children}]} ->
            if is_executable_code_block?(code_attrs, code_children) do
              process_executable_code_block({"pre", attrs, [{"code", code_attrs, code_children}]}, task_counter)
            else
              {element, task_counter}
            end
    ...
    case extract_code_info_from_container(children) do
      {:ok, code_info} ->
        if is_executable_language?(code_info.language) do
          enhanced_container = enhance_task_container({"div", attrs, children}, code_info, task_counter)
  ...
  defp process_executable_code_block({"pre", attrs, [{"code", code_attrs, code_children}]}, task_counter) do
    language = extract_language_from_code_attrs(code_attrs)
    code_text = Floki.text(code_children) |> String.trim()
  ...
  defp create_task_toolbar(task_id, code_info) do
    {
      "div",
      [
        {"class", "task-toolbar flex items-center justify-between bg-gradient-to-r from-green-50 to-blue-50 border-b border-green-200 px-4 py-2 text-sm"},
        {"role", "toolbar"},
        ...
        # Action buttons section
        {
          ...
          [
            # Run button
            {
              ...
              [
                {"type", "button"},
                {"class", "run-task-btn bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs font-medium flex items-center gap-1 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1"},
            ...
            # Stop button (initially hidden)
            {
              ...
              [
                {"type", "button"},
                {"class", "stop-task-btn hidden bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-xs font-medium flex items-center gap-1 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-1"},
  ...
  defp add_task_output_area(children, task_id) do
    output_area = {
      ...
      [
        {
          "div",
          [{"class", "output-header bg-gray-800 text-gray-200 px-4 py-2 text-xs font-medium flex items-center justify-between"}],
          [
            {
              "span",
              [],
        ...
        },
        {
          "pre",
          [
            {"class", "output-content bg-gray-900 text-gray-100 p-4 overflow-x-auto text-sm font-mono max-h-64 overflow-y-auto"},
