defmodule Kyozo.Workspaces.Extensions.RenderMarkdown.PostProcessors.HeadingProcessor do
  @moduledoc """
  ...
  defp text_to_slug(text) do
    text
    |> String.downcase()
    |> String.replace(~r/[^\w\s-]/, "")  # Remove special characters except word chars, spaces, hyphens
    ...
    |> case do
      "" -> "heading"  # Fallback for empty slugs
      slug -> slug
    end
  end
  ...
  defp create_heading_with_anchor(children, id, tag) do
    anchor_classes = 
      case tag do
        tag when tag in ["h1", "h2"] -> "text-lg opacity-0 group-hover:opacity-100 transition-opacity"
        _ -> "text-base opacity-0 group-hover:opacity-100 transition-opacity"
      end
