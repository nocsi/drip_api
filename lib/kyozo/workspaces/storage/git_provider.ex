defmodule Kyozo.Workspaces.Storage.GitProvider do
  ...
  def list(directory_path, options) do
    with :ok <- Storage.validate_options(options),
      ...
      case File.ls(full_path) do
        {:ok, files} ->
          metadata_list = Enum.map(files, fn file ->
  ...
  def list_versions(file_path, options) do
    with :ok <- Storage.validate_options(options),
      ...
      versions = Enum.map(commits, fn commit ->
        %{
          version: commit.sha,
  ...
  def sync(_from_backend, _to_backend, _file_path, _options) do
    # Git sync would involve pushing/pulling from remote repositories
    {:error, "Git sync not yet implemented"}
  end
  ...
  defp commit_changes(repo_path, file_path, options) do
    ...
    with {_, 0} <- System.cmd("git", ["add", file_path], cd: repo_path),
         {_, 0} <- System.cmd("git", ["commit", "-m", commit_message, "--author", "#{author} <#{author}@kyozo.dev>"], cd: repo_path),
