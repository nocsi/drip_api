defmodule Kyozo.Markdown.Transform do
  @moduledoc """
  ...
  def add_kyozo_data(node, kyozo_data) do
    ...
    current_kyozo = Map.get(current_data, :kyozo, %{})

    updated_kyozo = Map.merge(current_kyozo, kyozo_data)
    updated_data = Map.put(current_data, :kyozo, updated_kyozo)
  ...
  def build_execution_plan(ast) do
    ...
    blocks_with_deps =
      Enum.map(blocks, fn block ->
        deps = get_in(block, [:data, :kyozo, :metadata, "dependsOn"]) || []
        {block, deps}
      end)
  ...
  def to_execution_format(ast) do
    ...
    Enum.map(blocks, fn %AST.Code{} = block ->
      %{
        ...
        language: block.lang,
        content: block.value,
        executable: get_in(block, [:data, :kyozo, :executable]) || false,
        metadata: get_in(block, [:data, :kyozo, :metadata]) || %{},
