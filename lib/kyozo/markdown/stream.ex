defmodule Kyozo.Markdown.Stream do
  @moduledoc """
  ...
  alias Kyozo.Markdown.Stream.{Parser, Event}
  ...
  ## Examples
      ...
      stream
      |> Kyozo.Markdown.Stream.extract(:code_blocks)
      |> Enum.to_list()

      # Extract headings
      stream
      |> Kyozo.Markdown.Stream.extract(:headings)

      # Extract Kyozo metadata
      stream
      |> Kyozo.Markdown.Stream.extract(:kyozo_metadata)
  """
  ...
  ## Examples
      ...
      stream
      |> Kyozo.Markdown.Stream.inject_after(:heading, &generate_toc/1)

      # Inject enlightenment after code blocks
      stream
      |> Kyozo.Markdown.Stream.inject_after(:code_block, &enlighten/1)
  """
  ...
  ## Examples
      ...
      stream
      |> Kyozo.Markdown.Stream.transform(fn
      ...
      case event do
        {:comment, content} ->
          case parse_kyozo_comment(content) do
            {:ok, metadata} ->
              {[{:kyozo_metadata, metadata}], Map.put(state, :last_metadata, metadata)}
        ...
        {:start_code_block, meta} when is_map_key(state, :last_metadata) ->
          enhanced_meta = Map.put(meta, :kyozo, state.last_metadata)
...
defmodule Kyozo.Markdown.Stream.Parser do
  @moduledoc """
...
defmodule Kyozo.Markdown.Stream.Event do
  @moduledoc """
  ...
  def extractor_state(:kyozo_metadata), do: %{}
  ...
  def extract({:end_code_block, _}, %{extracting: true, current: current} = state) do
    code_block = %{
      ...
      kyozo: current.meta[:kyozo]
    }

    {[code_block], %{state | extracting: false, current: nil}}
  end

  def extract({:kyozo_metadata, metadata}, state) do
    {[metadata], state}
  end
