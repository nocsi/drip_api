defmodule Kyozo.Polyglot.Executors do
  @moduledoc """
  Executors that actually run polyglot markdown based on its type.
  """

  defmodule Behaviour do
    @callback execute(Kyozo.Polyglot.t()) :: {:ok, any()} | {:error, term()}
  end
  ...
  defmodule Docker do
    ...
    def execute(%{artifacts: artifacts} = polyglot) do
      with {:ok, %{dockerfile: content}} <- Kyozo.Polyglot.transpile(polyglot, :docker),
  ...
  defmodule Terraform do
    ...
    def execute(%{} = polyglot) do
      with {:ok, %{configuration: config}} <- Kyozo.Polyglot.transpile(polyglot, :terraform),
  ...
  defmodule Kubernetes do
    ...
    def execute(%{} = polyglot) do
      with {:ok, %{manifests: manifests}} <- Kyozo.Polyglot.transpile(polyglot, :kubernetes) do
  ...
  defmodule Git do
    ...
    def execute(%{} = polyglot) do
      with {:ok, %{files: files, init_commands: commands}} <-
             Kyozo.Polyglot.transpile(polyglot, :git),
