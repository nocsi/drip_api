defmodule DirupWeb.API.TeamsController do
  ...
  def show(conn, %{"id" => id}) do
    ...
    with {:ok, team} <-
           Accounts.get_team(id, actor: current_user, load: [:user_teams, :invitations]) do
  ...
  def invite_member(conn, %{"team_id" => team_id, "invitation" => invitation_params}) do
    ...
    with {:ok, invitation} <-
           Accounts.invite_user(invitation_params, actor: current_user, tenant: team_id) do
      conn
      |> put_status(:created)
  ...
  def remove_member(conn, %{"team_id" => team_id, "member_id" => member_id}) do
    ...
    with {:ok, _} <-
           Accounts.remove_team_member(%{id: member_id}, actor: current_user, tenant: team_id) do
      send_resp(conn, :no_content, "")
    end
  end
  ...
  def update_member_role(conn, %{"team_id" => team_id, "member_id" => member_id, "role" => role}) do
    ...
    with {:ok, member} <-
           Accounts.change_member_role(%{id: member_id, role: role},
             actor: current_user,
             tenant: team_id
           ) do
  ...
  def invitations(conn, %{"team_id" => team_id}) do
    ...
    with {:ok, invitations} <-
           Accounts.list_invitations_sent(actor: current_user, tenant: team_id) do
  ...
  def accept_invitation(conn, %{"invitation_id" => invitation_id}) do
    ...
    with {:ok, _invitation} <-
           Accounts.accept_invitation(%{id: invitation_id}, actor: current_user) do
      send_resp(conn, :no_content, "")
    end
  end
  ...
  def decline_invitation(conn, %{"invitation_id" => invitation_id}) do
    ...
    with {:ok, _invitation} <-
           Accounts.decline_invitation(%{id: invitation_id}, actor: current_user) do
      send_resp(conn, :no_content, "")
    end
  end
  ...
  def cancel_invitation(conn, %{"team_id" => team_id, "invitation_id" => invitation_id}) do
    ...
    with {:ok, _invitation} <-
           Accounts.cancel_invitation(%{id: invitation_id}, actor: current_user, tenant: team_id) do
      send_resp(conn, :no_content, "")
    end
  end
