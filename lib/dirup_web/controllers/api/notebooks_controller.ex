defmodule DirupWeb.API.NotebooksController do
  ...
  def create_from_document(conn, %{"document_id" => document_id, "notebook" => notebook_params}) do
    ...
    with {:ok, notebook} <-
           Workspaces.create_from_document(
             params,
             actor: current_user,
             tenant: current_team
           ) do
  ...
  def update(conn, %{"id" => id, "notebook" => notebook_params}) do
    ...
    with {:ok, notebook} <-
           Workspaces.get_notebook(id, actor: current_user, tenant: current_team),
         {:ok, updated_notebook} <-
           Workspaces.update_content(
             notebook,
             notebook_params,
             actor: current_user,
             tenant: current_team
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, notebook} <-
           Workspaces.get_notebook(id, actor: current_user, tenant: current_team),
         {:ok, _notebook} <-
           Workspaces.destroy_notebook(notebook, actor: current_user, tenant: current_team) do
  ...
  def execute(conn, %{"id" => id}) do
    ...
    with {:ok, notebook} <-
           Workspaces.get_notebook(id, actor: current_user, tenant: current_team),
         {:ok, executed_notebook} <-
           Workspaces.execute_notebook(
             notebook,
             %{
  ...
  def tasks(conn, %{"id" => id}) do
    ...
    with {:ok, notebook} <-
           Workspaces.get_notebook(id, actor: current_user, tenant: current_team),
         {:ok, tasks} <-
           Workspaces.list_notebook_tasks(notebook.id, actor: current_user, tenant: current_team) do
  ...
  def workspace_tasks(conn, %{"workspace_id" => workspace_id}) do
    ...
    with {:ok, tasks} <-
           Workspaces.list_workspace_tasks(workspace_id,
             actor: current_user,
             tenant: current_team
           ) do
