defmodule DirupWeb.PageController do
  ...
  def workspaces(conn, _params) do
    ...
    if current_user do
      ...
      teams =
        case Dirup.Workspaces.list_user_teams(actor: current_user) do
          {:ok, teams} -> teams
          _ -> []
        end
      ...
      |> render(:workspaces, layout: {DirupWeb.Layouts, :app})
    else
  ...
  def teams(conn, _params) do
    ...
    if current_user do
      ...
      |> render(:teams, layout: {DirupWeb.Layouts, :app})
    else
      redirect(conn, to: ~p"/login")
    end
  end
  ...
  def maybe_put_development_users(conn) do
    users =
      if @env == :dev do
        Application.get_env(:dirup, :session)[:dev_users]
        |> Enum.map(&Dirup.Accounts.get_by(email: to_string(&1)))
      end
