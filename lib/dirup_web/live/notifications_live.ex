defmodule DirupWeb.NotificationsLive do
  use DirupWeb, :live_view

  on_mount {DirupWeb.LiveUserAuth, :current_user}

  def mount(_params, _session, socket) do
    notifications = Dirup.Accounts.notifications_for_user!(actor: socket.assigns.current_user)

    if connected?(socket) do
      "notifications:#{socket.assigns.current_user.id}"
      |> DirupWeb.Endpoint.subscribe()
    end
  ...
  def handle_event("dismiss-notification", %{"id" => id}, socket) do
    ...
    Dirup.Accounts.dismiss_notification(
      notification,
