defmodule DirupWeb.Live.Notebook.Editor do
  @moduledoc """
    ...
    case load_notebook_with_content(notebook_id, user) do
      {:ok, notebook, content} ->
        ...
        if connected?(socket) do
          Phoenix.PubSub.subscribe(Dirup.PubSub, "notebook:#{notebook_id}")
          Phoenix.PubSub.subscribe(Dirup.PubSub, "notebook:#{notebook_id}:execution")
          Phoenix.PubSub.subscribe(Dirup.PubSub, "notebook:#{notebook_id}:collaboration")
        end
  ...
  def handle_event("save_notebook", %{"content" => content}, socket) do
    case save_notebook_content(socket, content) do
      {:ok, updated_notebook} ->
        notebook_data =
          prepare_notebook_data(updated_notebook, content, socket.assigns.current_user)
  ...
  def handle_event("toggle_collaborative_mode", %{"enabled" => enabled}, socket) do
    case Workspaces.update_notebook(
  ...
  defp save_notebook_content(socket, content, html \\ nil) do
    ...
    case DocumentBlobRef.update_document_content(
      ...
      {:ok, _blob_ref} ->
        ...
        notebook_attrs =
          if rendered_html && String.length(rendered_html) > 0 do
  ...
  defp render_markdown_content(content) do
    try do
      ...
      case Application.get_env(:dirup, :markdown_processor, :earmark) do
        :mdex ->
