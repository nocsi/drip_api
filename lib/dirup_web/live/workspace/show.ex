defmodule DirupWeb.Live.Workspace.Show do
  @moduledoc """
  ...
  def mount(%{"id" => id}, _, socket) do
    ...
    case Dirup.Workspaces.get_workspace(id,
           actor: user,
           load: [:documents, :notebooks, :team, :created_by]
         ) do
      {:ok, workspace} ->
        documents = Dirup.Workspaces.list_documents(workspace_id: workspace.id, actor: user)
        notebooks = Dirup.Workspaces.list_notebooks(workspace_id: workspace.id, actor: user)
        ...
        if connected?(socket) do
          Dirup.Workspaces.subscribe(workspace.team_id)
        end
  ...
  def handle_event("delete_document", %{"id" => id}, socket) do
    case Dirup.Workspaces.delete_document(id, actor: socket.assigns.current_user) do
      {:ok, document} ->
        socket =
  ...
  def handle_event("delete_notebook", %{"id" => id}, socket) do
    case Dirup.Workspaces.destroy_notebook(id, actor: socket.assigns.current_user) do
      {:ok, notebook} ->
        socket =
  ...
  def handle_event("archive_workspace", _, socket) do
    case Dirup.Workspaces.archive_workspace(socket.assigns.workspace.id,
  ...
  def handle_event("restore_workspace", _, socket) do
    case Dirup.Workspaces.restore_workspace(socket.assigns.workspace.id,
           actor: socket.assigns.current_user
         ) do
      {:ok, workspace} ->
        socket =
