...
defmodule DirupWeb.Accounts.Groups.GroupsLive do
  use DirupWeb, :live_view
  import DirupWeb.Components.Link
  import DirupWeb.Components.Modal
  import DirupWeb.Components.Table
  ...
  def render(assigns) do
    ...
    <div class="flex justify-between">
      ...
      <DirupWeb.Accounts.Groups.GroupForm.form actor={@current_user} id={Ash.UUIDv7.generate()} />
    </div>
    <%!-- Table groups --%>
      ...
      <:action :let={group}>
        <div class="space-x-6">
          ...
          >
            ...
            {gettext("Edit")}
            <DirupWeb.Accounts.Groups.GroupForm.form actor={@current_user} id={group.id} />
          </.modal>
  ...
  defp maybe_subscribe(socket) do
    if connected?(socket), do: DirupWeb.Endpoint.subscribe("groups")
  ...
  defp get_groups(actor) do
    Ash.read!(Dirup.Accounts.Group, actor: actor)
  end
end
