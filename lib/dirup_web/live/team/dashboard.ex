defmodule DirupWeb.Live.Team.Dashboard do
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      Phoenix.PubSub.subscribe(DirupWeb.PubSub, "members:#{current_user.id}")
      Phoenix.PubSub.subscribe(DirupWeb.PubSub, "invitations:#{current_user.id}")
    end
  ...
  def handle_event("select_team", %{"team_id" => team_id}, socket) do
    {:noreply,
     push_patch(socket,
       to: ~p"/team/dashboard?tab=#{socket.assigns.current_tab}&team_id=#{team_id}"
     )}
  end
    ...
    case Accounts.get_user_by_email(email) do
      {:ok, user} ->
        case Accounts.invite_user(%{invited_user_id: user.id},
               ...
               tenant: socket.assigns.selected_team
             ) do
          {:ok, _invitation} ->
            socket =
              socket
  ...
  def handle_event("leave_team", %{"team_id" => team_id}, socket) do
    ...
    case Enum.find(
           ...
           &(&1.team_id == team_id && &1.user_id == socket.assigns.current_user.id)
         ) do
      nil ->
  ...
  defp load_team_data(socket, team_id) when is_binary(team_id) do
    ...
    sent_invitations =
      Accounts.list_invitations_sent(actor: socket.assigns.current_user, tenant: team_id)
    ...
    <!-- Team Members -->
          <div class="px-6 py-4">
            <h4 class="text-md font-medium text-gray-900 mb-4">
              Team Members ({length(@team_members || [])})
            </h4>
    ...
    <!-- Sent Invitations -->
          <%= if length(@sent_invitations || []) > 0 do %>
            <div class="px-6 py-4 border-t border-gray-200">
              <h4 class="text-md font-medium text-gray-900 mb-4">
                Pending Invitations ({length(@sent_invitations)})
              </h4>
              <div class="space-y-3">
                  ...
                  <div class="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 class="text-sm font-medium text-gray-900">
                        Invited to {invitation.team.name}
                      </h4>
                      <p class="text-sm text-gray-500">
                        From {invitation.inviter_user.name || invitation.inviter_user.email}
                      </p>
