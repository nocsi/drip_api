defmodule Dirup.Containers.ServiceInstance do
  use Ash.Resource,
    otp_app: :dirup,
  ...
  actions do
    ...
    create :create do
      ...
      change {Dirup.Containers.Changes.SetTeamFromWorkspace, []}
      change {Dirup.Containers.Changes.ValidateDeploymentConfig, []}
    end
    ...
    update :deploy do
      ...
      change {Dirup.Containers.Changes.StartContainerDeployment, []}
    end

    update :start do
      accept []
      change {Dirup.Containers.Changes.StartContainer, []}
      change set_attribute(:status, :running)
    end

    update :stop do
      accept []
      change {Dirup.Containers.Changes.StopContainer, []}
    ...
    update :scale do
      ...
      change {Dirup.Containers.Changes.ScaleService, []}
  ...
  validations do
    ...
    validate {Dirup.Containers.Validations.ValidatePortMappings, []}
    validate {Dirup.Containers.Validations.ValidateResourceLimits, []}
  end
  ...
  attributes do
    ...
    attribute :folder_manifest_ld, :map do
      public? true
      description "JSON-LD manifest of folder as infrastructure"
    end

    attribute :infrastructure_graph, :map do
      public? true
      description "Dependency graph derived from folder structure"
    end

    attribute :content_hash, :string do
      public? true
      description "Hash of folder contents for differencing"
    end
  ...
  relationships do
    ...
    has_many :dependencies, Dirup.Containers.ServiceDependency do
      public? true
      destination_attribute :dependent_service_id
    end

    has_many :dependents, Dirup.Containers.ServiceDependency do
      public? true
      destination_attribute :required_service_id
    end

    # Monitoring
    has_many :deployment_events, Dirup.Containers.DeploymentEvent do
      public? true
      sort created_at: :desc
    end

    has_many :health_checks, Dirup.Containers.HealthCheck do
      public? true
      sort created_at: :desc
    end

    has_many :metrics, Dirup.Containers.ServiceMetric do
      public? true
    end
  end

  calculations do
    calculate :uptime_seconds, :integer, {Dirup.Containers.Calculations.Uptime, []}

    calculate :deployment_status, :string, {Dirup.Containers.Calculations.DeploymentStatus, []}

    calculate :resource_utilization, :map, {Dirup.Containers.Calculations.ResourceUtilization, []}
  end
