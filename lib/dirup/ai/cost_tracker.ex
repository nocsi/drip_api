...
#       filter expr(fragment("DATE(?)", inserted_at) == ^arg(:date))
#     end
...
#     |> Ash.Query.for_read(:recent_usage, %{days: 1})
#     |> Ash.read!()
#     |> Enum.filter(fn record ->
#       record_date = DateTime.to_date(record.inserted_at)
#       Date.compare(record_date, today) == :eq
#     end)
...
#           Enum.filter(results, &(&1.workspace_id == ws_id))

#         {nil, t_id} when not is_nil(t_id) ->
#           Enum.filter(results, &(&1.team_id == t_id))
