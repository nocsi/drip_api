defmodule Dirup.Containers do
  ...
  resources do
    ...
    resource Dirup.Containers.ServiceInstance do
    ...
    resource Dirup.Containers.TopologyDetection do
    ...
    resource Dirup.Containers.DeploymentEvent do
      define :list_deployment_events, action: :read
      define :get_deployment_event, action: :read, get_by: :id
    end

    resource Dirup.Containers.ServiceDependency do
  ...
  def subscribe(topic) when is_binary(topic) do
    Phoenix.PubSub.subscribe(Dirup.PubSub, "containers:#{topic}")
  end

  def subscribe(service_instance_id) do
    Phoenix.PubSub.subscribe(Dirup.PubSub, "containers:#{service_instance_id}")
  end
  ...
  def subscribe_to_deployments(service_instance_id) do
    Phoenix.PubSub.subscribe(Dirup.PubSub, "deployments:#{service_instance_id}")
  end
  ...
  def subscribe_to_health_checks(service_instance_id) do
    Phoenix.PubSub.subscribe(Dirup.PubSub, "health:#{service_instance_id}")
  end
  ...
  def broadcast(topic, event, payload) do
    Phoenix.PubSub.broadcast(Dirup.PubSub, "containers:#{topic}", {event, payload})
  end
  ...
  def broadcast_deployment(service_instance_id, event, payload) do
    Phoenix.PubSub.broadcast(Dirup.PubSub, "deployments:#{service_instance_id}", {event, payload})
  end
  ...
  def broadcast_health_check(service_instance_id, status, payload) do
    Phoenix.PubSub.broadcast(Dirup.PubSub, "health:#{service_instance_id}", {status, payload})
  end
end
