defmodule Dirup.Storage.Workers do
  @moduledoc """
  ...
  defmodule ProcessStorageWorker do
    ...
    defp get_storage_provider(:git), do: Dirup.Storage.Providers.Git
    defp get_storage_provider(:s3), do: Dirup.Storage.Providers.S3
    defp get_storage_provider(:disk), do: Dirup.Storage.Providers.Disk
    defp get_storage_provider(:ram), do: Dirup.Storage.Providers.RAM
    defp get_storage_provider(:hybrid), do: Dirup.Storage.Providers.Hybrid
  end
    ...
    defp perform_bulk_cleanup(args) do
      ...
      case Ash.read(query) do
        {:ok, resources} ->
          ...
          if error_count > 0 do
            {:error,
             "Bulk cleanup completed with errors: #{success_count} success, #{error_count} errors"}
          else
            :ok
          end
      ...
      if storage_resource.is_versioned do
        ...
        if content do
          ...
          if function_exported?(provider, :create_version, 3) do
            ...
            error_reason =
              "Storage backend #{storage_resource.storage_backend} does not support versioning"
  ...
  defmodule BulkProcessWorker do
    ...
    defp process_unprocessed(batch_size, backend_filter) do
      query =
        StorageResource
        |> Ash.Query.filter(
          storage_metadata["processed"] != true or is_nil(storage_metadata["processed"])
        )
    ...
    defp schedule_cleanup_jobs(args) do
      # 1 week default
      max_age_hours = get_arg(args, "max_age_hours") || get_arg(args, :max_age_hours) || 168
    ...
    defp perform_health_check(_args) do
      ...
      stats_result =
        case Dirup.Storage.get_operation_stats() do
          {:ok, stats} -> {:ok, stats}
