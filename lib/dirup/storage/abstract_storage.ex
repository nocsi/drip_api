defmodule Dirup.Storage.AbstractStorage do
  @moduledoc """
  ...
  alias Dirup.Storage.{Locator, Upload, StorageResource}
  ...
  defmacro __using__(opts) do
    ...
    domain = Keyword.get(opts, :domain, Dirup.Storage)

    quote do
      @behaviour Dirup.Storage.AbstractStorage
      ...
      alias Dirup.Storage.{AbstractStorage, StorageResource, Locator, Upload}

      # Import common storage functionality
      import Dirup.Storage.AbstractStorage.CommonActions
      import Dirup.Storage.AbstractStorage.CommonPolicies
      import Dirup.Storage.AbstractStorage.CommonCalculations
      import Dirup.Storage.AbstractStorage.CommonValidations
      ...
      unquote(Dirup.Storage.AbstractStorage.base_attributes())

      # Standard storage resource relationships
      unquote(Dirup.Storage.AbstractStorage.base_relationships())

      # Standard storage actions
      unquote(Dirup.Storage.AbstractStorage.base_actions())

      # Standard storage policies
      unquote(Dirup.Storage.AbstractStorage.base_policies())
    end
  end
  ...
  def base_relationships do
    quote do
      relationships do
        belongs_to :storage_resource, Dirup.Storage.StorageResource do
          allow_nil? false
    ...
    quote do
      actions do
        ...
        create :create_from_content do
          ...
          change {Dirup.Storage.AbstractStorage.Changes.ProcessContent, media_type: @media_type}

          change {Dirup.Storage.AbstractStorage.Changes.CreateStorageResource,
                  backends: @storage_backends}

          change {Dirup.Storage.AbstractStorage.Changes.SetupRelationship, []}
        end
        ...
        create :create_from_upload do
          ...
          change {Dirup.Storage.AbstractStorage.Changes.ProcessUpload, media_type: @media_type}
        ...
        create :create_from_locator do
          ...
          change {Dirup.Storage.AbstractStorage.Changes.CreateFromLocator, media_type: @media_type}
        end
        ...
        update :set_as_primary do
          ...
          change {Dirup.Storage.AbstractStorage.Changes.ClearOtherPrimary, []}
        end

        action :get_content, :map do
          run {Dirup.Storage.AbstractStorage.Actions.GetContent, []}
        end
        ...
        action :duplicate_to_backend, :struct do
          ...
          run {Dirup.Storage.AbstractStorage.Actions.DuplicateToBackend, media_type: @media_type}
        end

        action :process_content, :struct do
          argument :processing_options, :map, default: %{}

          run {Dirup.Storage.AbstractStorage.Actions.ProcessContent, media_type: @media_type}
        end

        action :cleanup_expired, {:array, :struct} do
          run {Dirup.Storage.AbstractStorage.Actions.CleanupExpired, []}
        end
      end
    end
  end
  ...
  defmodule CommonValidations do
    def validate_media_type do
      quote do
        validate {Dirup.Storage.AbstractStorage.Validations.ValidateMediaType,
    ...
    def validate_storage_backend do
      quote do
        validate {Dirup.Storage.AbstractStorage.Validations.ValidateStorageBackend,
                  backends: @storage_backends}
      end
    end
  end
