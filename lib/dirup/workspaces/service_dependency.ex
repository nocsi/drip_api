defmodule Dirup.Workspaces.ServiceDependency do
  ...
  actions do
    ...
    action :validate_dependency_graph, :map do
      ...
      run {Dirup.Workspaces.ServiceDependency.Actions.ValidateDependencyGraph, []}
    end

    action :get_startup_order, {:array, :uuid} do
      argument :workspace_id, :uuid, allow_nil?: false

      run {Dirup.Workspaces.ServiceDependency.Actions.GetStartupOrder, []}
    end

    action :check_service_readiness, :boolean do
      argument :service_instance_id, :uuid, allow_nil?: false

      run {Dirup.Workspaces.ServiceDependency.Actions.CheckServiceReadiness, []}
    end
  ...
  relationships do
    belongs_to :dependent_service, Dirup.Workspaces.ServiceInstance do
      allow_nil? false
      attribute_writable? true
    end

    belongs_to :required_service, Dirup.Workspaces.ServiceInstance do
      allow_nil? false
      attribute_writable? true
    end
