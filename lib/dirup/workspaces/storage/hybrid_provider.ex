defmodule Dirup.Workspaces.Storage.HybridProvider do
  @moduledoc """
  ...
  @behaviour Dirup.Workspaces.Storage

  alias Dirup.Workspaces.Storage
  alias Dirup.Workspaces.Storage.GitProvider
  alias Dirup.Workspaces.Storage.S3Provider
  ...
  def retrieve(file_path, options) do
    ...
    backend =
      case Keyword.get(options, :preferred_backend) do
  ...
  def delete(file_path, options) do
    ...
    case {git_result, s3_result} do
      ...
      # Deleted from Git successfully
      {:ok, {:error, _}} ->
        :ok

      # Deleted from S3 successfully
      {{:error, _}, :ok} ->
        :ok

      {{:error, git_error}, {:error, s3_error}} ->
        Logger.error(
          "Failed to delete #{file_path} from both backends. Git: #{inspect(git_error)}, S3: #{inspect(s3_error)}"
        )
  ...
  defp determine_backend_for_retrieval(file_path, options) do
    cond do
      ...
      true ->
        # Default to Git for retrieval attempts
        :git
    end
  end
  ...
  defp should_backup_to_s3?(file_path, content) do
    ...
    byte_size(content) > 1024 * 1024 or
      Path.extname(file_path) in [".ipynb", ".md"] or
      String.contains?(file_path, "README")
  end
  ...
  defp create_s3_backup(file_path, content, options, git_metadata) do
    ...
    case S3Provider.store(backup_path, content, options) do
      ...
      {:error, reason} ->
        ...
        # Return original metadata, backup failure is not critical
        {:ok, git_metadata}
    end
  end
  ...
  defp safe_list(provider, directory_path, options) do
    case provider.list(directory_path, options) do
      {:ok, files} -> {:ok, files}
      # Return empty list if one backend fails
      {:error, _reason} -> {:ok, []}
    end
  end
