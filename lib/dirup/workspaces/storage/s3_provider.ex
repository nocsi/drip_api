defmodule Dirup.Workspaces.Storage.S3Provider do
  @moduledoc """
  ...
  defp sync_s3_to_backend(file_path, target_backend, options) do
    ...
    with {:ok, s3_object} <- get_s3_object(bucket, file_path, region) do
      ...
      case target_backend do
        :git ->
          Dirup.Workspaces.Storage.GitProvider.store(file_path, s3_object.body, target_options)

        :disk ->
          Dirup.Workspaces.Storage.DiskProvider.store(file_path, s3_object.body, target_options)
  ...
  defp sync_backend_to_s3(file_path, source_backend, options) do
    ...
    source_result =
      case source_backend do
        :git ->
          Dirup.Workspaces.Storage.GitProvider.retrieve(file_path, options)
  ...
  defp get_s3_encryption_config do
    Application.get_env(:dirup, :s3_encryption, nil)
  end
end
