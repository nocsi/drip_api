defmodule Dirup.Workspaces.Storage.GitProvider do
  @moduledoc """
  ...
  defp sync_git_to_backend(file_path, target_backend, options) do
    with {:ok, repo_path} <- get_repo_path(options),
      ...
      case target_backend do
        :s3 ->
          Dirup.Workspaces.Storage.S3Provider.store(file_path, content, target_options)

        :disk ->
          Dirup.Workspaces.Storage.DiskProvider.store(file_path, content, target_options)
  ...
  defp sync_backend_to_git(file_path, source_backend, options) do
    ...
    source_result =
      case source_backend do
        :s3 ->
          Dirup.Workspaces.Storage.S3Provider.retrieve(file_path, options)

        :disk ->
          Dirup.Workspaces.Storage.DiskProvider.retrieve(file_path, options)
  ...
  defp get_git_root do
    Application.get_env(:dirup, :git_storage_root, Path.join([System.tmp_dir!(), "kyozo", "git"]))
  end
end
