defmodule Dirup.Workspaces.ServiceInstance do
  ...
  actions do
    ...
    action :deploy, :struct do
      ...
      run {Dirup.Workspaces.ServiceInstance.Actions.DeployService, []}
    end

    action :start, :struct do
      constraints instance_of: __MODULE__
      run {Dirup.Workspaces.ServiceInstance.Actions.StartService, []}
    end
    ...
    action :stop, :struct do
      ...
      run {Dirup.Workspaces.ServiceInstance.Actions.StopService, []}
    end
    ...
    action :scale, :struct do
      ...
      run {Dirup.Workspaces.ServiceInstance.Actions.ScaleService, []}
    end
    ...
    action :get_logs, :map do
      ...
      run {Dirup.Workspaces.ServiceInstance.Actions.GetServiceLogs, []}
    end
    ...
    action :get_metrics, :map do
      ...
      run {Dirup.Workspaces.ServiceInstance.Actions.GetServiceMetrics, []}
    end

    action :get_health_status, :map do
      run {Dirup.Workspaces.ServiceInstance.Actions.GetHealthStatus, []}
    end
  ...
  relationships do
    ...
    belongs_to :diruplogy_detection, Dirup.Workspaces.TopologyDetection do
      allow_nil? true
      attribute_writable? true
    end

    has_many :deployment_events, Dirup.Workspaces.DeploymentEvent do
      destination_attribute :service_instance_id
    end

    has_many :dependent_services, Dirup.Workspaces.ServiceDependency do
      destination_attribute :dependent_service_id
    end

    has_many :required_by_services, Dirup.Workspaces.ServiceDependency do
  ...
  calculations do
    ...
    calculate :can_deploy, :boolean, {Dirup.Calculations.CanPerformAction, action: :deploy}
    calculate :can_start, :boolean, {Dirup.Calculations.CanPerformAction, action: :start}
    calculate :can_stop, :boolean, {Dirup.Calculations.CanPerformAction, action: :stop}
    calculate :can_scale, :boolean, {Dirup.Calculations.CanPerformAction, action: :scale}
  end
