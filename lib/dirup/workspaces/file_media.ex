defmodule Dirup.Workspaces.FileMedia do
  ...
  def storage_options(backend, metadata) do
    ...
    case backend do
      ...
      :ram ->
        Map.merge(base_options, %{
          # 1 hour TTL for cached thumbnails
          ttl: 3600,
          # 10MB max for RAM storage
  ...
  relationships do
    ...
    belongs_to :media, Dirup.Workspaces.Media do
      allow_nil? true
      attribute_writable? true
      public? true
    end
              ...
              case Dirup.Storage.store_content(content, file.name,
                ...
                {:error, reason} ->
                  Ash.Changeset.add_error(
                    changeset,
                    "Failed to store image content: #{inspect(reason)}"
                  )
              end
      ...
      def run(file_media, input, _context) do
        ...
        thumbnails =
          Enum.map(sizes, fn size ->
            ...
            # Placeholder
            %{size: size, thumbnail: file_media}
          end)
