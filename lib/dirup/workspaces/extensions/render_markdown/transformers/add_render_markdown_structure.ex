defmodule Dirup.Workspaces.Extensions.RenderMarkdown.Transformers.AddRenderMarkdownStructure do
  @moduledoc """
  ...
  def transform(dsl) do
    ...
    result =
      Enum.reduce_while(render_attributes, {:ok, dsl}, fn {source, destination},
                                                          {:ok, current_dsl} ->
  ...
  defp transform_render_attribute(dsl, source, destination, extract_tasks?) do
    try do
      ...
      error ->
        {:error,
         "Failed to transform render attribute #{source} -> #{destination}: #{inspect(error)}"}
    end
  end
  ...
  defp add_render_markdown_change(dsl, source, destination, extract_tasks?) do
    change_entity =
      Transformer.build_entity!(
        ...
        :change,
        change: {
          Dirup.Workspaces.Extensions.RenderMarkdown.Changes.RenderMarkdown,
          source: source, destination: destination, extract_tasks?: extract_tasks?
        }
      )
    ...
    if Enum.any?(existing_calculations, &(&1.name == :markdown_stats)) do
      ...
      stats_calculation =
        Transformer.build_entity!(
          ...
          calculation: {
            Dirup.Workspaces.Extensions.RenderMarkdown.Calculations.MarkdownStats,
            []
          }
        )
