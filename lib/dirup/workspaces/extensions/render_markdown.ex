defmodule Dirup.Workspaces.Extensions.RenderMarkdown do
  @moduledoc """
  ...
  use Spark.Dsl.Extension,
    ...
    transformers: [
      Dirup.Workspaces.Extensions.RenderMarkdown.Transformers.AddRenderMarkdownStructure
    ]
  ...
  def as_html(text, opts) when is_binary(text) do
    ...
    case MDEx.to_html(processed_text, mdx_options) do
      {:ok, html_content} ->
        ...
        processed_html =
          Dirup.Workspaces.Extensions.RenderMarkdown.PostProcessor.run(
            html_content,
            add_ids?,
  ...
  defp process_line(
         ...
         acc_lines,
         acc_tasks,
         {current_lang, current_code, start_line, line_num},
         allowed_languages
       ) do
    line_num = line_num + 1
      ...
      String.starts_with?(line, "```") and not is_nil(current_lang) ->
        ...
        if current_lang in allowed_languages or Enum.empty?(allowed_languages) do
          ...
          task = %{
            ...
            # +1 because we don't include the ``` line
            line_start: start_line + 1,
            # -1 because we don't include the closing ``` line
            line_end: line_num - 1,
