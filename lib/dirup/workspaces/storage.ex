defmodule Dirup.Workspaces.Storage do
  @moduledoc """
  ...
  """
  @callback sync(
              from_backend :: storage_backend(),
              to_backend :: storage_backend(),
              file_path,
              storage_options
            ) ::
              {:ok, metadata} | {:error, reason :: any()}
  ...
  def build_workspace_storage_path(team_id, workspace_name) do
    normalized_name =
      String.downcase(workspace_name)
  ...
  """
  def build_storage_path(team_id, workspace_name)
      when is_binary(team_id) and is_binary(workspace_name) do
  ...
  def get_provider(backend) do
    case backend do
      :git -> Dirup.Workspaces.Storage.GitProvider
      :s3 -> Dirup.Workspaces.Storage.S3Provider
      :hybrid -> Dirup.Workspaces.Storage.HybridProvider
  ...
  def create_backup(file_path, content, options \\ []) do
    ...
    with {:ok, primary_metadata} <-
           get_provider(primary_backend).store(file_path, content, options),
