defmodule Dirup.Workspaces.Workspace do
  ...
  # events do
  #   event :workspace_created, Dirup.Workspaces.Events.WorkspaceCreated
  #   event :workspace_updated, Dirup.Workspaces.Events.WorkspaceUpdated
  #   event :workspace_deleted, Dirup.Workspaces.Events.WorkspaceDeleted
  #   event :workspace_archived, Dirup.Workspaces.Events.WorkspaceArchived
  #   event :workspace_restored, Dirup.Workspaces.Events.WorkspaceRestored
  # end
  ...
  actions do
    ...
    create :create_workspace do
      ...
      change {Dirup.Workspaces.Workspace.Changes.InitializeStorage, []}
      change {Dirup.Workspaces.Workspace.Changes.CreateInitialFiles, []}

      after_action(
        {Dirup.Workspaces.Workspace.Changes.EmitWorkspaceEvent, event: :workspace_created}
      )
    end
    ...
    create :seed_workspace do
      ...
      change set_attribute(:status, :active)
      change {Dirup.Workspaces.Workspace.Changes.SeedStoragePath, []}
    end

    update :update_workspace do
      accept [:name, :description, :tags, :settings]

      change {Dirup.Workspaces.Workspace.Changes.ValidateSettings, []}

      after_action(
        {Dirup.Workspaces.Workspace.Changes.EmitWorkspaceEvent, event: :workspace_updated}
      )
    end
    ...
    update :archive_workspace do
      ...
      after_action(
        {Dirup.Workspaces.Workspace.Changes.EmitWorkspaceEvent, event: :workspace_archived}
      )
    end
    ...
    update :restore_workspace do
      ...
      after_action(
        {Dirup.Workspaces.Workspace.Changes.EmitWorkspaceEvent, event: :workspace_restored}
      )
    end
    ...
    update :change_storage_backend do
      ...
      change {Dirup.Workspaces.Workspace.Changes.MigrateStorage, []}
    ...
    action :get_storage_info, :map do
      run {Dirup.Workspaces.Workspace.Changes.GetStorageInfo, []}
    end

    action :get_statistics, :map do
      run {Dirup.Workspaces.Workspace.Changes.GetWorkspaceStatistics, []}
    end

    action :cleanup_storage, :struct do
      argument :dry_run, :boolean, default: true

      run {Dirup.Workspaces.Workspace.Changes.CleanupStorage, []}
    end
    ...
    action :duplicate_workspace, :struct do
      ...
      run {Dirup.Workspaces.Workspace.Changes.DuplicateWorkspace, []}
    end
    ...
    destroy :soft_delete do
      ...
      after_action(
        {Dirup.Workspaces.Workspace.Changes.EmitWorkspaceEvent, event: :workspace_deleted}
      )
    end
    ...
    destroy :hard_delete do
      ...
      change {Dirup.Workspaces.Workspace.Changes.DeleteStorage, []}
  ...
  changes do
    ...
    # change before_action({Dirup.Workspaces.Workspace.Changes.BuildStoragePath, []}), on: [:create]
    # change before_action({Dirup.Workspaces.Workspace.Changes.NormalizeSettings, []}), on: [:create, :update]
    change after_action({Dirup.Workspaces.Workspace.Changes.UpdateStorageMetadata, []}),
      on: [:update]
  end
  ...
  validations do
    ...
    validate {Dirup.Workspaces.Workspace.Validations.ValidateStorageBackend, []}
    validate {Dirup.Workspaces.Workspace.Validations.ValidateSettings, []}
    validate {Dirup.Workspaces.Workspace.Validations.UniqueNamePerTeam, []}
  end
  ...
  relationships do
    # belongs_to :team_member, Dirup.Workspace.TeamMember do
    #   allow_nil? false
    # end
    ...
    belongs_to :created_by, Dirup.Accounts.User do
      allow_nil? true
      attribute_writable? true
    end

    has_many :files, Dirup.Workspaces.File do
      destination_attribute :workspace_id
    end

    has_many :notebooks, Dirup.Workspaces.Notebook do
      destination_attribute :workspace_id
    end

    has_many :load_events, Dirup.Workspaces.LoadEvent do
