defmodule Dirup.Workspaces.Workspace.Changes do
  @moduledoc """
  ...
  defmodule InitializeStorage do
    ...
    def change(changeset, _opts, context) do
      ...
      storage_path = Dirup.Workspaces.Storage.build_storage_path(team_id, workspace_name)
  ...
  defmodule CreateInitialFiles do
    ...
    defp create_initial_readme(workspace, context) do
      ...
      - Versioning: #{if Dirup.Workspaces.Workspace.supports_versioning?(workspace), do: "Enabled", else: "Disabled"}
      - Binary Files: #{if Dirup.Workspaces.Workspace.supports_binary?(workspace), do: "Supported", else: "Not Supported"}
      """

      # Create README document
      Dirup.Workspaces.create_document!(
        %{
          title: "README.md",
    ...
    def change(changeset, _opts, _context) do
      case Ash.Changeset.get_attribute(changeset, :settings) do
        ...
        settings when is_map(settings) ->
          if Dirup.Workspaces.Workspace.valid_settings?(settings) do
            changeset
          else
  ...
  defmodule MigrateStorage do
    ...
    def change(changeset, _opts, context) do
      Ash.Changeset.after_action(changeset, fn changeset, workspace ->
        ...
        updated_workspace =
          ...
          |> Dirup.Workspaces.update_workspace!()
    ...
    defp migrate_workspace_files(workspace, new_backend, context) do
      ...
      documents =
        Dirup.Workspaces.list_documents!(
          query: [filter: [workspace_id: workspace.id]],
          tenant: workspace.team_id
        )

      notebooks =
        Dirup.Workspaces.list_notebooks!(
          query: [filter: [workspace_id: workspace.id]],
          tenant: workspace.team_id
        )
    ...
    defp migrate_file(file, from_backend, to_backend, context) do
      try do
        ...
        old_provider = Dirup.Workspaces.Storage.get_provider(from_backend)

        {:ok, content, metadata} =
          old_provider.retrieve(
            file.file_path,
        ...
        new_provider = Dirup.Workspaces.Storage.get_provider(to_backend)
  ...
  defmodule GetStorageInfo do
    ...
    def run(input, _opts, context) do
      ...
      storage_info = %{
        ...
        storage_path: Dirup.Workspaces.Workspace.storage_path(workspace),
        storage_metadata: workspace.storage_metadata,
        supports_versioning: Dirup.Workspaces.Workspace.supports_versioning?(workspace),
        supports_binary: Dirup.Workspaces.Workspace.supports_binary?(workspace),
    ...
    defp calculate_total_size(workspace) do
      files_size =
        ...
        |> Dirup.Workspaces.read_one!()

      notebooks_size =
        Dirup.Workspaces.Notebook
        |> Ash.Query.filter(workspace_id == ^workspace.id)
        |> Ash.Query.filter(is_nil(deleted_at))
        |> Ash.Query.aggregate(:sum, :file_size)
        |> Dirup.Workspaces.read_one!()
  ...
  defmodule GetWorkspaceStatistics do
    ...
    def run(input, _opts, _context) do
      workspace =
        input.resource
        |> Ash.load!([:document_count, :notebook_count, :total_size, :last_activity])
    ...
    defp get_storage_usage(workspace) do
      ...
      # 1GB default
      max_size = get_in(workspace.settings, ["max_workspace_size"]) || 1_073_741_824

      %{
        used_bytes: total_size,
        max_bytes: max_size,
        usage_percentage:
          if(max_size > 0, do: Float.round(total_size / max_size * 100, 2), else: 0),
  ...
  defmodule DeleteStorage do
    ...
    defp delete_workspace_storage(workspace, context) do
      try do
        provider = Dirup.Workspaces.Workspace.storage_provider(workspace)
        storage_path = Dirup.Workspaces.Workspace.storage_path(workspace)
        ...
        error ->
          ...
          Logger.error(
            "Failed to delete storage for workspace #{workspace.id}: #{inspect(error)}"
          )
      end
    end
  end
  ...
  defmodule BuildStoragePath do
    ...
    @impl true
    def change(changeset, _opts, _context) do
      case {Ash.Changeset.get_attribute(changeset, :team_id),
            Ash.Changeset.get_attribute(changeset, :name)} do
        {team_id, name} when not is_nil(team_id) and not is_nil(name) ->
          normalized_name = Dirup.Workspaces.Workspace.normalize_name(name)
          storage_path = Dirup.Workspaces.Storage.build_storage_path(team_id, normalized_name)
  ...
  defmodule NormalizeSettings do
    ...
    def change(changeset, _opts, _context) do
      ...
      default_settings = Dirup.Workspaces.Workspace.default_settings()
  ...
  defmodule EmitWorkspaceEvent do
    ...
    defp emit_event(:workspace_created, workspace) do
      Dirup.Workspaces.Events.WorkspaceCreated.emit(%{
        workspace_id: workspace.id,
        team_id: workspace.team_id,
        name: workspace.name,
        storage_backend: workspace.storage_backend,
    ...
    defp emit_event(:workspace_updated, workspace) do
      Dirup.Workspaces.Events.WorkspaceUpdated.emit(%{
        workspace_id: workspace.id,
        team_id: workspace.team_id,
        name: workspace.name,
        updated_at: workspace.updated_at
    ...
    defp emit_event(:workspace_deleted, workspace) do
      Dirup.Workspaces.Events.WorkspaceDeleted.emit(%{
        workspace_id: workspace.id,
        team_id: workspace.team_id,
        name: workspace.name,
    ...
    defp emit_event(:workspace_archived, workspace) do
      Dirup.Workspaces.Events.WorkspaceArchived.emit(%{
        workspace_id: workspace.id,
        team_id: workspace.team_id,
        name: workspace.name,
    ...
    defp emit_event(:workspace_restored, workspace) do
      Dirup.Workspaces.Events.WorkspaceRestored.emit(%{
        workspace_id: workspace.id,
        team_id: workspace.team_id,
        name: workspace.name,
