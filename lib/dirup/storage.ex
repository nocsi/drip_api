defmodule Dirup.Storage do
  ...
  ## Usage Examples
      ...
      # Store a file
      {:ok, storage_resource} = Dirup.Storage.store_file(content, "document.md", backend: :git)

      # Retrieve content
      {:ok, content} = Dirup.Storage.retrieve_content(storage_resource)

      # Create a new version
      {:ok, versioned_resource} = Dirup.Storage.create_version(storage_resource, new_content)
  """
  ...
  alias Dirup.Storage.{StorageResource, Upload, Locator}
  ...
  resources do
    # Existing resources
    resource Dirup.Storage.StorageResource
    resource Dirup.Storage.ContentAddress
  ...
  defdelegate list_files_with_virtual(workspace_id, path, opts \\ %{}),
    to: Dirup.Storage.VFS,
    as: :list_files

  defdelegate read_virtual_file(workspace_id, path), to: Dirup.Storage.VFS, as: :read_file
  ...
  ## Examples
      ...
      {:ok, job} = Dirup.Storage.schedule_storage_processing(storage_resource,
        content: "new content",
  ...
  def schedule_storage_processing(%StorageResource{} = storage_resource, opts \\ []) do
    ...
    Dirup.Storage.Workers.schedule_processing(
      storage_resource.id,
      content: content,
      storage_options: storage_options,
      priority: priority
  ...
  ## Examples
      ...
      {:ok, job} = Dirup.Storage.schedule_storage_cleanup(storage_resource,
        priority: 1
      )
  """
  def schedule_storage_cleanup(%StorageResource{} = storage_resource, opts \\ []) do
    ...
    Dirup.Storage.Workers.schedule_cleanup(
      storage_resource.id,
      priority: priority
    )
  end
  ...
  ## Examples
      ...
      {:ok, job} = Dirup.Storage.schedule_version_creation(storage_resource, content,
        version_name: "v2.0",
  ...
  def schedule_version_creation(%StorageResource{} = storage_resource, content, opts \\ []) do
    ...
    Dirup.Storage.Workers.schedule_version_creation(
      storage_resource.id,
      content: content,
      version_name: version_name,
      commit_message: commit_message,
  ...
  ## Examples
      ...
      {:ok, status} = Dirup.Storage.get_processing_status(storage_resource)
  ...
  ## Examples
      ...
      {:ok, cancelled_count} = Dirup.Storage.cancel_scheduled_operations(storage_resource)
  """
  ...
  ## Examples
      ...
      {:ok, schedules} = Dirup.Storage.get_schedule_info()
  ...
  ## Examples
      ...
      {:ok, stats} = Dirup.Storage.get_operation_stats()
  ...
  ## Examples
      ...
      {:ok, job} = Dirup.Storage.force_processing(storage_resource,
        content: "new content",
        storage_options: %{compression: true}
      )
  """
  ...
  ## Examples
      ...
      {:ok, job} = Dirup.Storage.force_version_creation(storage_resource,
        content: "new content",
        version_name: "v1.0",
  ...
  ## Examples
      ...
      {:ok, result} = Dirup.Storage.schedule_bulk_processing(
        operation: :process_unprocessed,
        batch_size: 100
      )

      # Schedule maintenance
      {:ok, result} = Dirup.Storage.schedule_maintenance(
