defmodule Dirup.MarkdownLD.StreamParser.Listener do
  @moduledoc """
  ...
  ## Implementation
      ...
      defmodule MyCustomListener do
        use Dirup.MarkdownLD.StreamParser.Listener
  ...
  alias Dirup.MarkdownLD.StreamParser.{Chunk, Context}
  ...
  defmacro __using__(opts \\ []) do
    quote location: :keep, bind_quoted: [opts: opts] do
      @behaviour Dirup.MarkdownLD.StreamParser.Listener
      ...
      defp broadcast_event(event_type, event_data, context, topic \\ "markdown_processing") do
        ...
        Phoenix.PubSub.broadcast(Dirup.PubSub, topic, {event_type, event_payload})
      end
      ...
      defp increment_metric(metric_name, value \\ 1, context \\ %{}) do
        :telemetry.execute(
          [:dirup, :markdown_ld, :listener, metric_name],
          %{value: value},
          context
        )
      end
      ...
      defp get_webhook_config do
        Application.get_env(:dirup, :markdown_processing_webhooks, %{enabled: false})
      end
