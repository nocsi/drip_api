defmodule Dirup.Projects.Services.ProjectLoader do
  @moduledoc """
  ...
  defp extract_code_blocks([line | rest], identity_mode, acc) do
    if String.starts_with?(line, "```") do
      ...
      {code_lines, remaining_lines, line_start} =
        extract_code_block_with_position(rest, [], length(acc) + 2)
  ...
  defp extract_code_block_with_position([], acc, line_start),
    do: {Enum.reverse(acc), [], line_start}
  ...
  defp generate_task_name(language, index) do
    language_name =
      case String.downcase(language || "code") do
        "py" -> "Python"
        "python" -> "Python"
        "js" -> "JavaScript"
