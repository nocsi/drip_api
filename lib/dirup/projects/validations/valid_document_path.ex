defmodule Dirup.Projects.Validations.ValidDocumentPath do
    ...
    case File.stat(absolute_path) do
      {:ok, _stat} ->
        case File.read(absolute_path) do
          ...
          {:error, :eacces} ->
            {:error,
             field: :absolute_path, message: "Document file is not readable (permission denied)"}

          {:error, reason} ->
            {:error, field: :absolute_path, message: "Cannot read document file: #{reason}"}
        end
  ...
  defp validate_is_file(absolute_path) do
    case File.stat(absolute_path) do
      ...
      {:ok, %File.Stat{type: :directory}} ->
        {:error, field: :absolute_path, message: "Path points to a directory, not a file"}

      {:ok, %File.Stat{type: type}} ->
        {:error, field: :absolute_path, message: "Path points to a #{type}, not a regular file"}

      {:error, reason} ->
        {:error, field: :absolute_path, message: "Cannot determine file type: #{reason}"}
    end
  end
    ...
    case File.stat(absolute_path) do
      {:ok, %File.Stat{size: size}} ->
        ...
        if size <= max_size do
          ...
          {:error,
           field: :absolute_path,
           message: "Document file is too large (#{size_mb}MB). Maximum allowed size is 50MB"}
        end
  ...
  defp validate_allowed_extension(absolute_path) do
    ...
    if extension in allowed_extensions do
      ...
      {:error,
       field: :absolute_path,
       message:
         "File type '#{extension}' is not supported. Allowed types: #{Enum.join(allowed_extensions, ", ")}"}
    end
  end
end
